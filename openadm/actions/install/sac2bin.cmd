...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Looks for binary files required by OpenADM and places them in <fs.BinDir>
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\sac2bin <-id:{<group-label>}> <-mode:{exec|test}>
@rem
@rem Disk locations:
...
@rem     - <fs.InstallDir> : sac2bin.<group-label>.ini
@rem
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\sac2bin-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=sac2bin-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log


  set input.ValueIsOk=%STATUS_ON%

  if not exist %fs.InstallDir%\sac2bin.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #
    @echo #   %fs.InstallDir%\sac2bin.%VALUE_LABELID%.ini
    @echo #
  ...
  :MAIN
    ...
    if /i {%VALUE_MODE%} EQU {%VALUE_MODE_EXEC%} (
      for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\sac2bin.%VALUE_LABELID%.ini) do (
        %sys.ColorDark%

        @echo   + %%i

        for /f "tokens=*" %%j in ('dir /s /b %%i') do (
          if exist "%%~dpnxj" (
            copy /v /y "%%~dpnxj" %fs.BinDir%  >  nul
