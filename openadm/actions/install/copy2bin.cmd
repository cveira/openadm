...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Takes binary files required by OpenADM and places them in <fs.BinDir>
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\copy2bin <-id:{<group-label>}> <-mode:{exec|test}>
@rem
@rem Disk locations:
...
@rem     - <fs.InstallDir> : copy2bin.<group-label>.ini
@rem
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\copy2bin-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=copy2bin-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log


  set input.ValueIsOk=%STATUS_ON%

  if not exist %fs.InstallDir%\copy2bin.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #
    @echo #   %fs.InstallDir%\copy2bin.%VALUE_LABELID%.ini
    @echo #
  ...
  :MAIN
    ...
    if /i {%VALUE_MODE%} EQU {%VALUE_MODE_EXEC%} (
      for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\copy2bin.%VALUE_LABELID%.ini) do (
        %sys.ColorDark%

        if exist "%%~i" (
          @echo   + "%%~i"

          @copy /v /y "%%~i" %fs.BinDir%  >  nul
