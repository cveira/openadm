...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Sequentially launches installation steps in order to make up the local system
@rem   as defined on the specified external configuration file.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   sleep.exe       - win2k3 reskit
@rem   list.exe        - win2k3 reskit
@rem   install.cmd     - openadm [install modules]
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\setup <-id:{<group-label>|list|xlist}>
@rem                       <-w:99>
@rem
@rem Disk locations:
...
@rem   setup -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : setup.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\setup-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=setup-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log


  if /i %context.SkipExec% EQU %STATUS_OFF% (
    set input.ValueIsOk=%STATUS_ON%

    if not exist %fs.InstallDir%\setup.%VALUE_LABELID%.ini set input.ValueIsOk=%STATUS_OFF%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo #
      @echo #   %fs.InstallDir%\setup.%VALUE_LABELID%.ini
      @echo #
  ...
  :MAIN
    ...
    set event.message="setup: processing group %VALUE_LABELID%. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "setup:%VALUE_LABELID% [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\setup.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\setup.*.ini
    ...
    for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\setup.%VALUE_LABELID%.ini) do (
      @echo   + %fs.SysOpDir%\%%i
      call %fs.SysOpDir%\%%i
      ...
      if %errorlevel% NEQ %EL_STATUS_OK% (
        ...
        set event.message="setup: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "setup:%VALUE_LABELID% [%SET_SSNSEQ%]" /d !event.message!  >  nul
      )
    ...
    :HELP
      ...
      @echo.
      @echo   setup [-id:{[group-label]^|list^|xlist}]
      @echo         [-w:99]
      @echo.
      @echo   ----------------------------------------------------------------
      @echo.
    ...
    :EXIT
      ...
      set event.message="setup: group %VALUE_LABELID% processed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "setup:%VALUE_LABELID% [%SET_SSNSEQ%]" /d !event.message! > nul
