...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Routes the user to the proper alert sub-command and/or launches a given sub-command.
@rem
@rem Dependencies:
@rem   colorx.exe      - http://internet.cybermesa.com/~bstewart/ (v2.0)
@rem   *.cmd           - openadm [alert modules]
@rem
@rem Usage:
@rem   <fs.InstallDir>\alert <operation-id>
@rem
@rem Disk locations:
...
@rem   fs.ModulesDir:     <fs.SystemDrive>\openadm\modules\alert
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
@rem   <operation-id>   - stands for an active module id.
@rem
@rem Important notes:
...
setlocal
  ...
  set fs.ModulesDir=%fs.SystemDrive%\openadm\modules\alert
  ...
  set VALUE_ALERTTYPE_REGULAR=regular
  set VALUE_ALERTTYPE_GROUPED=grouped


  if not exist %fs.InstallDir%\alert.ini (
    %sys.ColorRed%

    @echo.
    @echo # ERROR: input INI file does not exist.
    @echo #
    @echo #   %fs.ConfDir%\alert.ini
    @echo #
  ...
  :MAIN
    set id.IsNotFound=%STATUS_ON%

    for /f "eol=# tokens=1,2,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\alert.ini) do (
      if /i {%SET_ID%} EQU {%%i} (
        set id.IsNotFound=%STATUS_OFF%
        set id.AlertType=%%j
        set id.SetMailTrap=%%k
      )
    )
    ...
    if /i %id.AlertType% EQU %VALUE_ALERTTYPE_REGULAR% set input.ValueIsOk=%STATUS_ON%
    if /i %id.AlertType% EQU %VALUE_ALERTTYPE_GROUPED% set input.ValueIsOk=%STATUS_ON%
    ...
    if /i %id.SetMailTrap% EQU %STATUS_YES% set input.ValueIsOk=%STATUS_ON%
    if /i %id.SetMailTrap% EQU %STATUS_NO%  set input.ValueIsOk=%STATUS_ON%

    if /i %input.ValueIsOk% EQU %STATUS_OFF% goto HELP


    set VALUE_CMD=%*
    if /i %id.SetMailTrap% EQU %STATUS_NO%               set VALUE_CMD=%VALUE_CMD:-trap:yes=-trap:no%
    if /i %id.AlertType%   EQU %VALUE_ALERTTYPE_REGULAR% set VALUE_CMD=regular.cmd %VALUE_CMD%
    if /i %id.AlertType%   EQU %VALUE_ALERTTYPE_GROUPED% set VALUE_CMD=grouped.cmd %VALUE_CMD%

    if {%id.IsNotFound%} EQU {%STATUS_ON%} (
      goto HELP
    ) else (
      call %fs.ModulesDir%\%VALUE_CMD%
    )
