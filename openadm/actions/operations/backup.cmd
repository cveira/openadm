...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Routes the user to the proper backup sub-command and/or launches a given sub-command.
@rem
@rem Dependencies:
@rem   colorx.exe      - http://internet.cybermesa.com/~bstewart/ (v2.0)
@rem   *.cmd           - openadm [backup modules]
@rem
@rem Usage:
@rem   <fs.InstallDir>\backup <sub-command>
@rem
@rem Disk locations:
...
@rem   fs.ModulesDir:     <fs.SystemDrive>\openadm\modules\backup
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal
  ...
  set fs.ModulesDir=%fs.SystemDrive%\openadm\modules\backup
  ...
  if not exist %fs.InstallDir%\backup.ini (
    %sys.ColorRed%

    @echo.
    @echo # ERROR: input INI file does not exist.
    @echo #
    @echo #   %fs.ConfDir%\backup.ini
    @echo #
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\backup.ini) do (
      if /i %SET_CMD% EQU %%i (
