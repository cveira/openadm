...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Routes the user to the proper scan sub-command and/or launches a given sub-command.
@rem
@rem Dependencies:
@rem   colorx.exe      - http://internet.cybermesa.com/~bstewart/ (v2.0)
@rem   *.cmd           - openadm [scan modules]
@rem
@rem Usage:
@rem   <fs.InstallDir>\scan <sub-command>
@rem
@rem Disk locations:
...
@rem   fs.ModulesDir:     <fs.SystemDrive>\openadm\modules\scan
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal
  ...
  set fs.ModulesDir=%fs.SystemDrive%\openadm\modules\scan
  ...
  if not exist %fs.InstallDir%\scan.ini (
    %sys.ColorRed%

    @echo.
    @echo # ERROR: input INI file does not exist.
    @echo #
    @echo #   %fs.ConfDir%\scan.ini
    @echo #
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\scan.ini) do (
      if /i %SET_CMD% EQU %%i (
