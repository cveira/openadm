...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Routes the user to the proper clean sub-command and/or launches a given sub-command.
@rem
@rem Dependencies:
@rem   colorx.exe      - http://internet.cybermesa.com/~bstewart/ (v2.0)
@rem   *.cmd           - openadm [clean modules]
@rem
@rem Usage:
@rem   <fs.InstallDir>\clean <sub-command>
@rem
@rem Disk locations:
...
@rem   fs.ModulesDir:     <fs.SystemDrive>\openadm\modules\clean
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal
  ...
  set fs.ModulesDir=%fs.SystemDrive%\openadm\modules\clean
  ...
  if not exist %fs.InstallDir%\clean.ini (
    %sys.ColorRed%

    @echo.
    @echo # ERROR: input INI file does not exist.
    @echo #
    @echo #   %fs.ConfDir%\clean.ini
    @echo #
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\clean.ini) do (
      if /i %SET_CMD% EQU %%i (
