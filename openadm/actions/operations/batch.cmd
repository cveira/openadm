...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Routes the user to the proper batch sub-command and/or launches a given sub-command.
@rem
@rem Dependencies:
@rem   colorx.exe      - http://internet.cybermesa.com/~bstewart/ (v2.0)
@rem   *.cmd           - openadm [batch modules]
@rem
@rem Usage:
@rem   <fs.InstallDir>\batch <sub-command>
@rem
@rem Disk locations:
@rem   fs.InstallDir:     <fs.SystemDrive>\openadm\actions\operations
@rem   fs.BinDir:         <fs.SystemDrive>\openadm\bin\system
@rem   fs.ModulesDir:     <fs.SystemDrive>\openadm\modules\batch
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal
  ...
  set fs.ModulesDir=%fs.SystemDrive%\openadm\modules\batch
  ...
  if not exist %fs.InstallDir%\batch.ini (
    %sys.ColorRed%

    @echo.
    @echo # ERROR: input INI file does not exist.
    @echo #
    @echo #   %fs.ConfDir%\batch.ini
    @echo #
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\batch.ini) do (
      if /i %SET_CMD% EQU %%i (
