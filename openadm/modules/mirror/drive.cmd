...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Takes a snapshot from source volume into a file on destination folder.
@rem
@rem Dependencies:
@rem   dixml.exe       - http://www.runtime.org/dixml.htm (v1.18)
...
@rem   alert.cmd       - openadm [mirror.fs alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\mirror drive <source-dir>
@rem                              <destination-dir>
@rem                              <-id:group-label>
@rem                              <-rs:[1-99]>
@rem                              <-a:{yes|no}>
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\drive-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=volumes-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log


  set input.ValueIsOk=%STATUS_ON%

  if not exist %fs.ConfDir%\el.iddb.ini         set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\drive.ini        set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\db.mysql.ini) do (
      if /i "%%i" EQU "fs.DIXMLBinDir" set fs.DIXMLBinDir=%%j
    )
    ...
    if not exist !fs.DIXMLBinDir!\dixml.exe               set input.ValueIsOk=%STATUS_OFF%
    ...
    set event.message="mirror drive: [%VALUE_LABELID%] mirroring "!fs.SourceDir!" into "!fs.DestinationDir!". [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "mirror:drive [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_REPLICASETS% EQU 1 (
      ...
      %fs.DIXMLBinDir%\dixml /b%fs.SourceDir::=% /t"!fs.DestinationDir!\%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%"

      if !errorlevel! NEQ %EL_DIXML_OK% (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="mirror drive: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "mirror:drive [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!  >  nul
      )
      ...
      if !mirror.TokenCount! EQU 0 (
        set event.message="mirror drive: [%VALUE_LABELID%] replica set number is: 0. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "mirror:drive [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!  >  nul
        ...
        %fs.DIXMLBinDir%\dixml /b%fs.SourceDir::=% /t"!fs.DestinationDir!\rs0\%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%"
        ...
        for %%i in ("!fs.DestinationDir!\lastrs.*") do (
          ...
          if !fs.NewReplicaSet! LSS %VALUE_REPLICASETS% (
            set event.message="mirror drive: [%VALUE_LABELID%] replica set number is: !fs.NewReplicaSet!. [%fs.LogsDir%\%SET_LOGFILE%]"
            %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "mirror:drive [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!   >  nul
            ...
            %fs.DIXMLBinDir%\dixml /b%fs.SourceDir::=% /t"!fs.DestinationDir!\rs!fs.NewReplicaSet!\%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%"
    ...
    :ERROR
      ...
      set event.message="mirror drive: [%VALUE_LABELID%] invalid last replica set state token. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "mirror:drive [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!  >  nul
    ...
    :EXIT
      ...
      set event.message="mirror drive: [%VALUE_LABELID%] "!fs.SourceDir!" mirrored into "!fs.DestinationDir!". [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "mirror:drive [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd mirror.fs -l:%SET_LOGFILE% -trap:yes
