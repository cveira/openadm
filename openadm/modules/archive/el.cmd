...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Takes the contents from Event Log to an archive file and reset it in order
@rem   to get rid from old entries.
@rem
@rem Dependencies:
@rem   elsave.vbs      - openadm
...
@rem   alert.cmd       - openadm [archive.el alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\archive el <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ServicesDir:    <fs.SystemDrive>\openadm\services
...
setlocal enabledelayedexpansion
  ...
  set fs.ServicesDir=%fs.SystemDrive%\openadm\services
  ...
  set EL_ELSAVE_OK=0
  set EL_ELSAVE_ERROR=1
  set EL_RTELCOLLECT_OK=0
  set EL_RTELCOLLECT_ERROR=1
  set EL_WMIC_OK=0
  set EL_WMIC_ERROR=1
  ...
  if /i %SET_ALERTMODE:~0,5% NEQ -type goto HELP
  ...
  for %%i in (%fs.LogsDir%\el-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=el-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.ServicesDir%\rtelcollect.ini set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.ServicesDir%\rtelcollect.ini
    @echo #
  ...
  if not exist %fs.InstallDir%\elsave.vbs               set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    set event.message="archive el: archiving event logs. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "archive:el [%SET_SSNSEQ%]" /d !event.message!   >  nul
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.ConfDir%\el.local.ini) do (
      ...
      if /i "!property.name:~0,3!" EQU "el." (
        @echo   + "%%~j"
        %fs.InstallDir%\elsave.vbs "%%~j" > nul

        if %errorlevel% NEQ %EL_ELSAVE_OK% (
          @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
          ...
          set event.message="archive el: unexpected operation error ["%%~j"]. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "archive:el [%SET_SSNSEQ%]" /d !event.message!  >  nul
        ) else (
          set event.message="archive el: archived log ["%%~j"] successfully. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "archive:el [%SET_SSNSEQ%]" /d !event.message!  >  nul
        )
      )
    )
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # stopping rtelcollect service:
    @echo.

    %sys.ColorDark%

    wmic /locale:ms_409 process where "Name = 'cscript.exe' and CommandLine like '%rtelcollect.vbs%'" delete
    ...
    if %errorlevel% NEQ %EL_WMIC_OK% (
      ...
      set event.message="archive el: unexpected operation error stopping rtelcollect service. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "archive:el [%SET_SSNSEQ%]" /d !event.message!  >  nul
    ) else (
      set event.message="archive el: rtelcollect service stopped successfully. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "archive:el [%SET_SSNSEQ%]" /d !event.message!  >  nul
    )
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # starting rtelcollect service:
    @echo.

    %sys.ColorDark%

    for /f %%i in (%fs.ServicesDir%\rtelcollect.ini) do %fs.ServicesDir%\rtelcollect %%i

    if %errorlevel% NEQ %EL_RTELCOLLECT_OK% (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      set event.message="archive el: unexpected operation error starting rtelcollect service. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "archive:el [%SET_SSNSEQ%]" /d !event.message!  >  nul
    ) else (
      set event.message="archive el: rtelcollect service started successfully. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "archive:el [%SET_SSNSEQ%]" /d !event.message!  >  nul
    )
    ...
    :EXIT
      ...
      set event.message="archive el: event logs archived. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "archive:el [%SET_SSNSEQ%]" /d !event.message!  >  nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd archive.el -l:%SET_LOGFILE% -trap:yes
