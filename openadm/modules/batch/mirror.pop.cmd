...
@rem Date:        2009/01/18
@rem Script name: batch mirror.pop
@rem ------------------------------------------------------------------------------------
...
@rem   mirror.cmd      - openadm [pop module]
@rem   alert.cmd       - openadm [batch.mirror.pop module]
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\batch mirror.pop <-id:{<group-label>|list|xlist}>
@rem                                  <-w:99>
@rem
@rem Disk locations:
...
@rem   batch mirror.pop -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : mirror.pop.<group-label>.ini
@rem     - <fs.InstallDir> : mirror.pop.<group-label>.jobs.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\mirror.pop-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=mirror.pop-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if not exist %fs.InstallDir%\mirror.pop.%VALUE_LABELID%.ini       set input.ValueIsOk=%STATUS_OFF%
    if not exist %fs.InstallDir%\mirror.pop.%VALUE_LABELID%.jobs.ini  set input.ValueIsOk=%STATUS_OFF%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   %fs.InstallDir%\mirror.pop.%VALUE_LABELID%.ini
      @echo #   %fs.InstallDir%\mirror.pop.%VALUE_LABELID%.jobs.ini
      @echo #
      @echo # aborting program.
      @echo.
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\mirror.pop.%VALUE_LABELID%.ini) do (
      if /i {%%i} EQU {batch.RunAsynchronously} set batch.RunAsynchronously=%%~j
      if /i {%%i} EQU {batch.RunPriority}       set batch.RunPriority=%%~j
    )


    set event.message="batch mirror.pop: processing group %VALUE_LABELID%. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "batch:mirror.pop [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\mirror.pop.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\mirror.pop.*.ini
    ...
    if /i {!batch.RunAsynchronously!} EQU {%STATUS_NO%} (
      for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\mirror.pop.%VALUE_LABELID%.jobs.ini) do (
        @echo   + %fs.SysOpDir%\mirror.cmd pop %%i
        call %fs.SysOpDir%\mirror.cmd pop %%i
        ...
        if %errorlevel% NEQ %EL_STATUS_OK% (
          ...
          set event.message="batch mirror.pop: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "batch:mirror.pop [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
        ) else (
          set event.message="batch mirror.pop: [%VALUE_LABELID%] operation successfull [%fs.SysOpDir%\mirror.cmd pop %%i]. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "batch:mirror.pop [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
        )
    ...
    :HELP
      ...
      @echo.
      @echo   batch mirror.pop [-id:{[group-label]^|list^|xlist}]
      @echo                    [-w:99]
      @echo.
      @echo   ----------------------------------------------------------------
      @echo.
      ...
      @echo.
      @echo     batch mirror.pop -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="batch mirror.pop: group %VALUE_LABELID% processed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "batch:mirror.pop [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd batch.mirror.pop -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
