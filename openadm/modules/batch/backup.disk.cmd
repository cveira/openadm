...
@rem Date:        2009/01/18
@rem Script name: batch backup.disk
@rem ------------------------------------------------------------------------------------
...
@rem   backup.cmd      - openadm [disk module]
@rem   alert.cmd       - openadm [batch.backup.disk module]
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\batch backup.disk <-id:{<group-label>|list|xlist}>
@rem                                   <-w:99>
@rem
@rem Disk locations:
...
@rem   batch backup.disk -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on
@rem     - <fs.InstallDir> : backup.disk.<group-label>.ini
@rem     - <fs.InstallDir> : backup.disk.<group-label>.jobs.ini
...
setlocal enabledelayedexpansion
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if not exist %fs.InstallDir%\backup.disk.%VALUE_LABELID%.ini       set input.ValueIsOk=%STATUS_OFF%
    if not exist %fs.InstallDir%\backup.disk.%VALUE_LABELID%.jobs.ini  set input.ValueIsOk=%STATUS_OFF%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   %fs.InstallDir%\backup.disk.%VALUE_LABELID%.ini
      @echo #   %fs.InstallDir%\backup.disk.%VALUE_LABELID%.jobs.ini
      @echo #
      @echo # aborting program.
      @echo.
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\backup.disk.%VALUE_LABELID%.ini) do (
      if /i {%%i} EQU {batch.RunAsynchronously} set batch.RunAsynchronously=%%~j
      if /i {%%i} EQU {batch.RunPriority}       set batch.RunPriority=%%~j
    )

    
    set event.message="batch backup.disk: processing group %VALUE_LABELID%. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "batch:backup.disk [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\backup.disk.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\backup.disk.*.ini
    ...
    if /i {!batch.RunAsynchronously!} EQU {%STATUS_NO%} (
      for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\backup.disk.%VALUE_LABELID%.jobs.ini) do (
        @echo   + %fs.SysOpDir%\%%i
        call %fs.SysOpDir%\%%i
        ...
        if %errorlevel% NEQ %EL_STATUS_OK% (
          ...
          set event.message="batch backup.disk: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "batch:backup.disk [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
        ) else (
          set event.message="batch backup.disk: [%VALUE_LABELID%] operation successfull [%fs.SysOpDir%\%%i]. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "batch:backup.disk [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
        )
    ...
    :HELP
      ...
      @echo.
      @echo   batch backup.disk [-id:{[group-label]^|list^|xlist}]
      @echo                     [-w:99]
      @echo.
      @echo   ----------------------------------------------------------------
      @echo.
      ...
      @echo.
      @echo     batch backup.disk -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="batch backup.disk: group %VALUE_LABELID% processed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "batch:backup.disk [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd batch.backup.disk -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
