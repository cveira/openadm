...
@rem Date:        2009/01/18
@rem Script name: batch baseline
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Launches the commanded system baseline processes.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   sleep.exe       - win2k3 reskit
@rem   list.exe        - win2k3 reskit
@rem   baseline.cmd    - openadm [baseline modules]
@rem   alert.cmd       - openadm [batch.baseline module]
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\batch baseline <-id:{<group-label>|list|xlist}>
@rem                                <-w:99>
@rem
@rem Disk locations:
...
@rem   batch baseline -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : baseline.<group-label>.ini
@rem     - <fs.InstallDir> : baseline.<group-label>.jobs.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\baseline-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=baseline-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.SysOpDir%\baseline.cmd               set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if not exist %fs.InstallDir%\baseline.%VALUE_LABELID%.ini       set input.ValueIsOk=%STATUS_OFF%
    if not exist %fs.InstallDir%\baseline.%VALUE_LABELID%.jobs.ini  set input.ValueIsOk=%STATUS_OFF%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   %fs.InstallDir%\baseline.%VALUE_LABELID%.ini
      @echo #   %fs.InstallDir%\baseline.%VALUE_LABELID%.jobs.ini
      @echo #
      @echo # aborting program.
      @echo.
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\baseline.%VALUE_LABELID%.ini) do (
      if /i {%%i} EQU {batch.RunAsynchronously} set batch.RunAsynchronously=%%~j
      if /i {%%i} EQU {batch.RunPriority}       set batch.RunPriority=%%~j
    )


    set event.message="batch baseline: starting system baseline. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "batch:baseline [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\baseline.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\baseline.*.ini
    ...
    if /i {!batch.RunAsynchronously!} EQU {%STATUS_NO%} (
      for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\baseline.%VALUE_LABELID%.jobs.ini) do (
        @echo   + %fs.SysOpDir%\baseline.cmd %%i
        call %fs.SysOpDir%\baseline.cmd %%i
        ...
        if %errorlevel% NEQ %EL_STATUS_OK% (
          ...
          set event.message="batch baseline: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "batch:baseline [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
        ) else (
          set event.message="batch baseline: [%VALUE_LABELID%] operation successfull [%fs.SysOpDir%\baseline.cmd %%i]. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "batch:baseline [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
        )
    ...
    :HELP
      ...
      @echo.
      @echo   batch baseline [-id:{[group-label]^|list^|xlist}]
      @echo                  [-w:99]
      @echo.
      @echo   ----------------------------------------------------------------
      @echo.
      ...
      @echo.
      @echo     batch baseline -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="batch baseline: system baseline finished. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "batch:baseline [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd batch.baseline -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
