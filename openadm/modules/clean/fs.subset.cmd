...
@rem Date:        2009/01/18
@rem Script name: clean fs.subset
@rem ------------------------------------------------------------------------------------
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\clean fs.subset {none|<target-dir>}
...
@rem   <res-root>        - existing full-path directory name.
...
@rem   clean fs.subset none -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : fs.subset.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  if /i %context.SkipExec% EQU %STATUS_ON% (
    ...
    set VALUE_OPMODE=%VALUE_OPMODE_TEST%
    set VALUE_TIMETOWAIT=0
  )
  ...
  for %%i in (%fs.LogsDir%\fs.subset-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=fs.subset-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if not exist %fs.InstallDir%\fs.subset.%VALUE_LABELID%.ini           set input.ValueIsOk=%STATUS_OFF%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   %fs.InstallDir%\fs.subset.%VALUE_LABELID%.ini
      @echo #
      @echo # aborting program.
      @echo.
  ...
  :MAIN
    ...
    set event.message="clean fs.subset: [%VALUE_LABELID%] processing resource "!fs.TargetDir!". [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "clean:fs.subset [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\fs.subset.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\fs.subset.*.ini
    ...
    if /i %VALUE_OPMODE% NEQ %VALUE_OPMODE_TEST% (
      @echo # requested clean operations:
      @echo.
      ...
      for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\fs.subset.%VALUE_LABELID%.ini) do (
        for /d /r . %%k in ("*%%~j") do (
          @echo   + %fs.SysOpDir%\clean.cmd folder "%%~k" -id:%%i -mode:tree -a:yes
          call %fs.SysOpDir%\clean.cmd folder "%%~k" -id:%%i -mode:tree -a:yes
          ...
          if %errorlevel% NEQ %EL_CLEANFSDIR_OK% (
            ...
            @echo #
            @echo #   target resource: "%%~k"
            @echo #   resource label:  %%i
            @echo.

            %sys.ColorNormal%

            set event.message="clean fs.subset: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
            %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "clean:fs.subset [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
          )
      ...
      @echo # [test-mode] requested clean operations:
      @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   clean fs.subset [{none^|[target-dir]}]
      ...
      @echo.
      @echo     clean fs.subset none -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="clean fs.subset: [%VALUE_LABELID%] resource "!fs.TargetDir!" processed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "clean:fs.subset [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd clean.fs.subset -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
