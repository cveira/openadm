...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Dumps the contents of the MySQL Database.
@rem
@rem Dependencies:
@rem   mysqldump.exe   - mysql 5.x
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\dump db.mysql {none|<destination-dir>}
...
@rem   fs.DBBinDir:  [defined in db.db.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
@rem   dump db.mysql none -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : db.mysql.ini
@rem     - <fs.InstallDir> : db.mysql.<group-label>.ini
@rem     - <fs.InstallDir> : db.mysql.<group-label>.connection.ini
...
setlocal enabledelayedexpansion
  ...
  set EL_MYSQLDUMP_OK=0
  set EL_MYSQLDUMP_ERROR=1
  ...
  if /i %SET_DESTINATION:"=% NEQ %VALUE_DESTINATION_NONE% (
  ...
  for %%i in (%fs.LogsDir%\db.mysql-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=db.mysql-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log


  set input.ValueIsOk=%STATUS_ON%

  if not exist %fs.ConfDir%\el.iddb.ini         set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\db.mysql.ini     set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\db.mysql.ini
    @echo #
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if not exist %fs.InstallDir%\db.mysql.%VALUE_LABELID%.ini             set input.ValueIsOk=%STATUS_OFF%
    if not exist %fs.InstallDir%\db.mysql.%VALUE_LABELID%.connection.ini  set input.ValueIsOk=%STATUS_OFF%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   %fs.InstallDir%\db.mysql.%VALUE_LABELID%.ini
      @echo #   %fs.InstallDir%\db.mysql.%VALUE_LABELID%.connection.ini
      @echo #
      @echo # aborting program.
      @echo.
  ...
  for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\db.mysql.ini) do (
    if /i "%%i" EQU "fs.DBBinDir" set fs.DBBinDir=%%j
  )
  ...
  if not exist !fs.DBBinDir!\mysqldump.exe              set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   !fs.DBBinDir!\mysqldump.exe
    @echo #
    @echo # aborting program.
    @echo.
  ...
  :MAIN
    ...
    if /i %context.SkipExec% EQU %STATUS_OFF% (
      set db.SessionParameters=
      for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\db.mysql.%VALUE_LABELID%.ini) do (
        set db.SessionParameters=!db.SessionParameters! %%i
      )
    )


    set event.message="dump db.mysql: [%VALUE_LABELID%] dumping MySQL databases. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "dump:db.mysql [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\db.mysql.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\db.mysql.*.ini
    ...
    for /f "eol=# tokens=1,2,3,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\db.mysql.%VALUE_LABELID%.connection.ini) do (
      set db.SessionOutput=--result-file="!fs.DestinationDir!\db.mysql-%VALUE_LABELID%-%%i.%%j-%yy%%mm%%dd%-%fs.LogFileCount%.sql"
      set db.SessionHost=--host=%%i
      set db.SessionDb=--databases %%j
      set db.SessionUser=--user=%%k
      set db.SessionPassword=--password=%%l
      ...
      @%fs.DBBinDir%\mysqldump !db.SessionParameters! !db.SessionHost! !db.SessionUser! !db.SessionPassword! !db.SessionDb! !db.SessionOutput!

      if %errorlevel% NEQ %EL_MYSQLDUMP_OK% (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        @echo #   session.db:         %%j
        @echo #   session.user:       %%k
        @rem @echo #   session.pwd:        %%l
        @echo #   session.output:     "!fs.DestinationDir!\db.mysql-%VALUE_LABELID%-%%i.%%j-%yy%%mm%%dd%-%fs.LogFileCount%.sql"
        @echo #
        @echo #   session.parameters: !db.SessionParameters!
        @echo.

        %sys.ColorNormal%

        set event.message="dump db.mysql: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "dump:db.mysql [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    ...
    :HELP
      ...
      @echo.
      @echo   dump db.mysql [{none^|[destination-dir]}]
      ...
      @echo.
      @echo     dump db.mysql none -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="dump db.mysql: [%VALUE_LABELID%] MySQL databases dumped. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "dump:db.mysql [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd dump.db.mysql -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:yes
