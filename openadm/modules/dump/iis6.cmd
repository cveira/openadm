...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Dumps the contents of the IIS6 metabase.
@rem
@rem Dependencies:
@rem   iisback.vbs     - win2k3 server
...
@rem   alert.cmd       - openadm [dump.iis6 alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\dump iis6 <-id:group-label>
@rem                           <-a:{yes|no}>
@rem
@rem Disk locations:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\iis6-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=iis6-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\iisback.vbs              set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\iisback.vbs
    @echo #
    @echo # aborting program.
    @echo.
  ...
  :MAIN
    ...
    set fs.BackupFileName=dump.iis6-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%

    set event.message="dump iis6: [%VALUE_LABELID%] dumping metabase into %fs.BackupFileName%. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "dump:iis6 [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.InstallDir%\iisback.vbs /backup /b %fs.BackupFileName% /v NEXT_VERSION

    if %errorlevel% NEQ %EL_IISBACK_OK% (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      @echo #
      @echo #   backup name: %fs.BackupFileName%
      @echo.

      %sys.ColorNormal%

      set event.message="dump iis6: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "dump:iis6 [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    :EXIT
      ...
      set event.message="dump iis6: [%VALUE_LABELID%] metabase dumped into %fs.BackupFileName%. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "dump:iis6 [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd dump.iis6 -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:yes
