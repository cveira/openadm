...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Dumps the contents of the MQ Database.
@rem
@rem Dependencies:
@rem   mqbkup.exe      - win2k3 reskit
...
@rem   alert.cmd       - openadm [dump.mq alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\dump mq <destination-dir>
@rem                         <-id:group-label>
@rem                         <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.BinDir:     <fs.SystemDrive>\jobs\bin
@rem   fs.LogsDir:    <fs.DataDrive>\logs\jobs
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\mq-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=mq-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.BinDir%\mqbkup.exe                   set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    set fs.BackupFile="!fs.DestinationDir!\mq-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.bak"

    set event.message="dump mq: [%VALUE_LABELID%] dumping MQ into %fs.BackupFile%. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "dump:mq [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\mqbkup -b -y %fs.BackupFile%

    if %errorlevel% NEQ %EL_MQBKUP_OK% (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      set event.message="dump mq: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "dump:mq [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    :EXIT
      ...
      set event.message="dump mq: [%VALUE_LABELID%] MQ dumped into %fs.BackupFile%. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "dump:mq [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd dump.mq -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:yes
