...
@rem Date:        2009/01/18
@rem Script name: dump acl.services
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Dumps the Security Descriptor from the selected Services.
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\dump acl.services {none|<destination-dir>}
@rem                                   <-id:{<group-label>|all|list|xlist}>
@rem                                   <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
@rem   <none>            - no destination dir requred.
...
@rem   [-id:all]         - targets every object.
...
@rem
@rem Restricted/Shorcut parameter combination matrix:
@rem   dump acl.services none -id:{list|xlist}
@rem
@rem Important notes:
@@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : acl.services-<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\acl.services-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=acl.services-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  set fs.IniFile=%fs.InstallDir%\acl.services.%VALUE_LABELID%.ini

  if /i %context.SkipExec% EQU %STATUS_OFF% (
    if /i "%VALUE_LABELID%" NEQ "%VALUE_LABELID_ALL%" (
      set input.ValueIsOk=%STATUS_ON%

      if not exist %fs.InstallDir%\acl.services.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_OFF%
      ...
      if /i !input.ValueIsOk! EQU %STATUS_OFF% (
        ...
        @echo # ERROR: input INI file does not exist.
        @echo #
        @echo #   %fs.InstallDir%\acl.services.%VALUE_LABELID%.ini
        @echo #
        @echo # aborting program.
        @echo.
  ...
  :MAIN
    ...
    if /i %context.SkipExec% EQU %STATUS_OFF% (
      if /i "%VALUE_LABELID%" EQU "%VALUE_LABELID_ALL%" (
        wmic service get Name                                                         >>  %fs.TmpDir%\%SET_LOGFILE:.log=.tmp1%
        type %fs.TmpDir%\%SET_LOGFILE:.log=.tmp1% | %fs.BinDir%\findstr /v /c:"Name"  >>  %fs.TmpDir%\%SET_LOGFILE:.log=.tmp2%
        type %fs.TmpDir%\%SET_LOGFILE:.log=.tmp2% | %fs.BinDir%\sed -e "s/ \+$//"     >>  %fs.TmpDir%\%SET_LOGFILE:.log=.ini%

        del /q /f %fs.TmpDir%\%SET_LOGFILE:.log=%.tmp?                                >   nul

        set fs.IniFile=%fs.TmpDir%\%SET_LOGFILE:.log=.ini%
      )
    )


    set fs.BackupFile="%fs.DestinationDir%\acl.services-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.bak"

    set event.message="dump acl.services: [%VALUE_LABELID%] dumping file system objects security descriptors. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "dump:acl.services [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\acl.services.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\acl.services.*.ini
    ...
    for /f "eol=# tokens=*" %%i in (!fs.IniFile!) do (
      ...
      %fs.BinDir%\setacl -on "%%~i" -ot srv -actn list -lst "f:tab;w:d,s,o,g;i:y;s:b"
      %fs.BinDir%\setacl -on "%%~i" -ot srv -actn list -lst "f:sddl;w:d,s,o,g;i:y;s:b" | %fs.BinDir%\findstr /i /c:"O:"  >> "%fs.BackupFile%"
      ...
      if !errorlevel! NEQ %EL_SETACL_OK% (
        ...
        set event.message="dump acl.services: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "dump:acl.services [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    ...
    :HELP
      ...
      @echo.
      @echo   dump acl.services [{none^|[destination-dir]}]
      @echo                     [-id:{[group-label]^|all^|list^|xlist}]
      @echo                     [-a:{yes^|no}]
      @echo.
      @echo   ----------------------------------------------------------------
      @echo.
      @echo   Restricted/Shorcut parameter combination matrix:
      @echo.
      @echo     dump acl.services none -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="dump acl.services: [%VALUE_LABELID%] file system objects security descriptors dumped. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "dump:acl.services [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd dump.acl.services -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:yes
