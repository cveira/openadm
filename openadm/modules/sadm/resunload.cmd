...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Performs a bulk object unload from system resource.
@rem
@rem Dependencies:
...
@rem   alert.cmd       - openadm [resunload alert module]
@rem
@rem Usage:
@rem   <fs.InstallDir>\sadm resource -cmd:unload
...
@rem     - <%CD%>        : resunload-%VALUE_RESLABEL%.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\resunload-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=resunload-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  :MAIN
    ...
    if /i %VALUE_OPMODE% NEQ %VALUE_OPMODE_TEST% (
      set event.message="sadm res.unload.%VALUE_RESTYPE%: bulk ununloading objects on system resource [%VALUE_SOURCE%:%input.LabelName%:%VALUE_RESTYPE%:%VALUE_ACCESSROLE%:%VALUE_SCOPE%]"
      %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sadm:res.unload.%VALUE_RESTYPE% [%SET_SSNSEQ%]" /d !event.message! > nul
    ) else (
      set event.message="sadm res.unload.%VALUE_RESTYPE%: [test-mode] bulk ununloading objects on system resource [%VALUE_SOURCE%:%input.LabelName%:%VALUE_RESTYPE%:%VALUE_ACCESSROLE%:%VALUE_SCOPE%]"
      %fs.BinDir%\eventcreate /id %event.TestModeBeginId% /l application /t information /so "sadm:res.unload.%VALUE_RESTYPE% [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    if /i %VALUE_RESTYPE:~0,1% EQU %VALUE_RESTYPE_SYSPRIV% (
      ...
      set SecurityGroup.RootName=res-%input.LabelName%-%VALUE_ACCESSROLE%
    ...
    if /i %VALUE_OPMODE% NEQ %VALUE_OPMODE_TEST% (
      call %fs.ModulesDir%\resunload.UnloadGroups.cmd
    ) else (
      call %fs.ModulesDir%\resunload.QueryGroups.cmd
    )
    ...
    if /i %VALUE_OPMODE% NEQ %VALUE_OPMODE_TEST% (
      set event.message="sadm res.unload.%VALUE_RESTYPE%: objects bulk loaded on system resource [%VALUE_SOURCE%:%input.LabelName%:%VALUE_RESTYPE%:%VALUE_ACCESSROLE%:%VALUE_SCOPE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sadm:res.unload.%VALUE_RESTYPE% [%SET_SSNSEQ%]" /d !event.message! > nul
    ) else (
      set event.message="sadm res.unload.%VALUE_RESTYPE%: [test-mode] objects bulk loaded on system  resource [%VALUE_SOURCE%:%input.LabelName%:%VALUE_RESTYPE%:%VALUE_ACCESSROLE%:%VALUE_SCOPE%]"
      %fs.BinDir%\eventcreate /id %event.TestModeEndId% /l application /t information /so "sadm:res.unload.%VALUE_RESTYPE% [%SET_SSNSEQ%]" /d !event.message! > nul
    )


    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd resunload -l:%SET_LOGFILE% -trap:yes
