...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Queries the state of an existing local session for OpenADM Operations.
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.InstallDir>\sadm session -cmd:query <session-id>
@rem
@rem Disk locations:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\ssnquery-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=ssnquery-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  :MAIN
    ...
    set event.message="ssnquery: querying OpenADM session on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sadm:ssn.query [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if %OpenADM.SessionExists% EQU %STATUS_OFF% (
      ...
      set event.message="ssnquery: session  doesn't exist. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "sadm:ssn.query [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul

      goto EXIT
    )

    if exist %fs.CmdbDir%\%SET_SESSIONID% (
      @echo # session [%SET_SESSIONID%] is closed

      %sys.ColorDark%
      @echo   + %fs.CmdbDir%\%SET_SESSIONID%
      %sys.ColorNormal%
    ) else (
      @echo # session [%SET_SESSIONID%] is opened
    ...
    :HELP
      ...
      @echo.
      @echo   sadm session -cmd:query [session-id]
      @echo.
    ...
    :EXIT
      ...
      set event.message="ssnquery: OpenADM session queried on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sadm:ssn.query [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
