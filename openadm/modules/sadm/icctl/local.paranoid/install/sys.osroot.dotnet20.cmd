if /i %VALUE_OPMODE% NEQ %VALUE_OPMODE_TEST% (
  ...
  for /f "eol=# tokens=*" %%i in (%fs.ModulesDir%\icctl\%icctl.profile%\conf\sys.osroot.dotnet20.ini) do (
    if exist "%systemroot%\microsoft.net\framework\v2.0.50727\%%i" (
      ...
      :: %fs.BinDir%\setacl -on "%systemroot%\microsoft.net\framework\v2.0.50727\%%i" -ot file -actn ace %ACL_LOCAL_SYSTEM_PROFILE% %ACL_LOCAL_AUDIT_SYSTEM% -ace "n:%COMPUTERNAME%\res-ts-sysdotnet-rx;p:read_ex" -ace "n:%COMPUTERNAME%\res-ts-sysdotnet-da;p:read_ex;m:deny" -actn clear -clr "dacl,sacl"
  ...
  for /f "eol=# tokens=*" %%i in (%fs.ModulesDir%\icctl\%icctl.profile%\conf\sys.osroot.dotnet20.ini) do (
    if exist "%systemroot%\microsoft.net\framework\v2.0.50727\%%i" (
      ...
      :: @echo %fs.BinDir%\setacl -on "%systemroot%\microsoft.net\framework\v2.0.50727\%%i" -ot file -actn ace %ACL_LOCAL_SYSTEM_PROFILE% %ACL_LOCAL_AUDIT_SYSTEM% -ace "n:%COMPUTERNAME%\res-ts-sysdotnet-rx;p:read_ex" -ace "n:%COMPUTERNAME%\res-ts-sysdotnet-da;p:read_ex;m:deny" -actn clear -clr "dacl,sacl"
