if /i %VALUE_OPMODE% NEQ %VALUE_OPMODE_TEST% (
  ...
  %fs.BinDir%\setacl -on "e:\logs" -ot file -actn ace %ACL_LOCAL_SYSTEM_PROFILE% %ACL_LOCAL_AUDIT_SYSTEM% -ace "n:%COMPUTERNAME%\role-job-local;p:change" -ace "n:%COMPUTERNAME%\res-fs-logs;p:change" -ace "n:%COMPUTERNAME%\res-fs-logs-da;p:read_ex;m:deny" -ace "n:%COMPUTERNAME%\res-fs-logs-rx;p:read_ex" -actn clear -clr "dacl,sacl"
  ...
  @echo %fs.BinDir%\setacl -on "e:\logs" -ot file -actn ace %ACL_LOCAL_SYSTEM_PROFILE% %ACL_LOCAL_AUDIT_SYSTEM% -ace "n:%COMPUTERNAME%\role-job-local;p:change" -ace "n:%COMPUTERNAME%\res-fs-logs;p:change" -ace "n:%COMPUTERNAME%\res-fs-logs-da;p:read_ex;m:deny" -ace "n:%COMPUTERNAME%\res-fs-logs-rx;p:read_ex" -actn clear -clr "dacl,sacl"
