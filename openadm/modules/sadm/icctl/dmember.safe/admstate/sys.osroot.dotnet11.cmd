if /i %VALUE_OPMODE% NEQ %VALUE_OPMODE_TEST% (
  ...
  for /f "eol=# tokens=*" %%i in (%fs.ModulesDir%\icctl\%icctl.profile%\conf\sys.osroot.dotnet11.ini) do (
    if exist "%systemroot%\microsoft.net\framework\v1.1.4322\%%i" (
      ...
      %fs.BinDir%\subinacl /file "%systemroot%\microsoft.net\framework\v1.1.4322\%%i" /grant=system=f /grant=builtin\administrators=f
  ...
  for /f "eol=# tokens=*" %%i in (%fs.ModulesDir%\icctl\%icctl.profile%\conf\sys.osroot.dotnet11.ini) do (
    if exist "%systemroot%\microsoft.net\framework\v1.1.4322\%%i" (
      ...
      @echo %fs.BinDir%\subinacl /file "%systemroot%\microsoft.net\framework\v1.1.4322\%%i" /grant=system=f /grant=builtin\administrators=f
    )
  )
)