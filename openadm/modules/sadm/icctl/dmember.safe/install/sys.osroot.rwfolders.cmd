if /i %VALUE_OPMODE% NEQ %VALUE_OPMODE_TEST% (
  ...
  for /f "eol=# tokens=*" %%i in (%fs.ModulesDir%\icctl\%icctl.profile%\conf\sys.osroot.rwfolders.ini) do (
    if exist "%systemroot%\%%i" (
      ...
      %fs.BinDir%\setacl -on "%systemroot%\%%i" -ot file -actn ace %ACL_LOCAL_SYSTEM_PROFILE%
  ...
  for /f "eol=# tokens=*" %%i in (%fs.ModulesDir%\icctl\%icctl.profile%\conf\sys.osroot.rwfolders.ini) do (
    if exist "%systemroot%\%%i" (
      ...
      @echo %fs.BinDir%\setacl -on "%systemroot%\%%i" -ot file -actn ace %ACL_LOCAL_SYSTEM_PROFILE%
