...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Closes an existing and already opened local session for OpenADM Operations.
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.InstallDir>\sadm session -cmd:close <session-id>
@rem
@rem Disk locations:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\ssnclose-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=ssnclose-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  :MAIN
    ...
    set event.message="ssnclose: closing OpenADM session on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sadm:ssn.close [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if not exist %fs.CmdbDir%\%SET_SESSIONID%.open (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%

      %sys.ColorRed%
      @echo # WARNING: session already closed.
      %sys.ColorNormal%

      set event.message="ssnclose: session already closed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "sadm:ssn.close [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if exist %fs.CmdbDir%\%SET_SESSIONID% (
      ren %fs.CmdbDir%\%SET_SESSIONID%.open %fs.CmdbDir%\%SET_SESSIONID%  >  nul
    )
    ...
    :HELP
      ...
      @echo.
      @echo   sadm session -cmd:close [session-id]
      @echo.
    ...
    :EXIT
      ...
      set event.message="ssnclose: OpenADM session closed on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sadm:ssn.close [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
