...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Deletes an existing local session for OpenADM Operations regardless of its state.
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.InstallDir>\sadm session -cmd:delete <session-id>
@rem
@rem Disk locations:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\ssndelete-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=ssndelete-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  :MAIN
    ...
    set event.message="ssndelete: deleting OpenADM session on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sadm:ssn.delete [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    set OpenADM.SessionExists=%STATUS_OFF%
    if exist %fs.CmdbDir%\%SET_SESSIONID%      set OpenADM.SessionExists=%STATUS_ON%
    if exist %fs.CmdbDir%\%SET_SESSIONID%.open set OpenADM.SessionExists=%STATUS_ON%

    if %OpenADM.SessionExists% EQU %STATUS_OFF% (
      ...
      set event.message="ssndelete: session  doesn't exist. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "sadm:ssn.delete [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if exist %fs.CmdbDir%\%SET_SESSIONID% (
      @echo   + %fs.CmdbDir%\%SET_SESSIONID%
      del /f /q %fs.CmdbDir%\%SET_SESSIONID%       >  nul
      rd  /s /q %fs.CmdbDir%\%SET_SESSIONID%       >  nul

    ) else (
      @echo   + %fs.CmdbDir%\%SET_SESSIONID%.open
      del /f /q %fs.CmdbDir%\%SET_SESSIONID%.open  >  nul
      rd  /s /q %fs.CmdbDir%\%SET_SESSIONID%.open  >  nul
    )
    ...
    :HELP
      ...
      @echo.
      @echo   sadm session -cmd:delete [session-id]
      @echo.
    ...
    :EXIT
      ...
      set event.message="ssndelete: OpenADM session deleted on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sadm:ssn.delete [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
