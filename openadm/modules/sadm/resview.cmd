...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Queries the system looking for OpenADM resources.
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.InstallDir>\sadm resource -cmd:view
...
@rem                                 <-role:{any|rx|rw|rwx|rwxd|wo|ro|lo|da}>
@rem                                 <-scope:{local|ntdomain|addomain}>
@rem                                 <-view:{default|full}>
@rem                                 <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem   [-role:any]        - returns any access role defined for a given resource.
...
@rem   [-view:default]    - it sets regular view mode: performs enumeration of the objects already present on the system.
@rem   [-view:full]       - it sets full view mode: performs enumeration of the objects already present on the system.
...
setlocal enalbedelayedexpansion
  ...
  set VALUE_SCOPE=%SET_VIEWMODE:-scope:=%
  set VALUE_VIEWMODE=%SET_VIEWMODE:-view:=%
  ...
  set VALUE_VIEWMODE_DEFAULT=default
  set VALUE_VIEWMODE_FULL=full
  ...
  for %%i in (%fs.LogsDir%\resview-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=resview-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if /i %SET_VIEWMODE:~0,5%   NEQ -view  goto HELP
  ...
  if /i %VALUE_ACCESSROLE% EQU %VALUE_ACCESSROLE_ANY%  set input.ValueIsOk=%STATUS_ON%
  ...
  if /i %VALUE_VIEWMODE% EQU %VALUE_VIEWMODE_DEFAULT%  set input.ValueIsOk=%STATUS_ON%
  if /i %VALUE_VIEWMODE% EQU %VALUE_VIEWMODE_FULL%     set input.ValueIsOk=%STATUS_ON%
  ...
  :MAIN
    ...
    set event.message="sadm res.view.%VALUE_RESTYPE%: retrieving objects from selected system resource [%VALUE_RESLABEL%:%VALUE_RESTYPE%:%VALUE_ACCESSROLE%:%VALUE_SCOPE%:%VALUE_VIEWMODE%:%VALUE_ALERTMODE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sadm:res.view.%VALUE_RESTYPE% [%VALUE_RESLABEL%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_RESTYPE:~0,1% EQU %VALUE_RESTYPE_SYSPRIV% (
      if /i %VALUE_RESTYPE% EQU %VALUE_RESTYPE_ROLE%   set SecurityGroup.RootName=¡%VALUE_RESTYPE:¡=%-adm-%VALUE_RESLABEL%-%VALUE_ACCESSROLE%

      if %scope.IsLocal% EQU %STATUS_OFF% (
        if /i %VALUE_RESTYPE% EQU %VALUE_RESTYPE_SVC%  set SecurityGroup.RootName=¡sys-%VALUE_RESTYPE:¡=%-%VALUE_RESLABEL%-%VALUE_ACCESSROLE%
        if /i %VALUE_RESTYPE% EQU %VALUE_RESTYPE_JOB%  set SecurityGroup.RootName=¡sys-%VALUE_RESTYPE:¡=%-%VALUE_RESLABEL%-%VALUE_ACCESSROLE%
      ) else (
        if /i %VALUE_RESTYPE% EQU %VALUE_RESTYPE_SVC%  set SecurityGroup.RootName=¡role-%VALUE_RESTYPE:¡=%-%VALUE_RESLABEL%-%VALUE_ACCESSROLE%
        if /i %VALUE_RESTYPE% EQU %VALUE_RESTYPE_JOB%  set SecurityGroup.RootName=¡role-%VALUE_RESTYPE:¡=%-%VALUE_RESLABEL%-%VALUE_ACCESSROLE%
      )
    ) else (
      set SecurityGroup.RootName=res-%VALUE_RESLABEL%-%VALUE_ACCESSROLE%

      if /i %VALUE_RESTYPE% EQU %VALUE_RESTYPE_ORG%    set SecurityGroup.RootName=org-%VALUE_RESLABEL%
      if /i %VALUE_RESTYPE% EQU %VALUE_RESTYPE_ROLE%   set SecurityGroup.RootName=role-adm-%VALUE_RESLABEL%-%VALUE_ACCESSROLE%
    ...
    if /i %VALUE_ACCESSROLE% EQU %VALUE_ACCESSROLE_RWXD% set SecurityGroup.RootName=%SecurityGroup.RootName:-any=%


    call %fs.ModulesDir%\resview.QueryGroups.cmd
    call %fs.ModulesDir%\resview.QueryResources.cmd
    call %fs.ModulesDir%\resview.QueryCMDB.cmd
    ...
    if exist %fs.TmpDir%\%SET_LOGFILE:.log=.exit% (
      ...
      set event.message="sadm res.view.%VALUE_RESTYPE%: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "sadm:res.view.%VALUE_RESTYPE% [%VALUE_RESLABEL%] [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    :HELP
      ...
      @echo     [-role:{any^|rx^|rw^|rwx^|rwxd^|wo^|ro^|lo^|da}]
      @echo     [-scope:{local^|ntdomain^|addomain}]
      @echo     [-view:{default^|full}]
      @echo     [-a:{yes^|no}]
      @echo.
    ...
    set event.message="sadm res.view.%VALUE_RESTYPE%: objects from selected system resource retrieved [%VALUE_RESLABEL%:%VALUE_RESTYPE%:%VALUE_ACCESSROLE%:%VALUE_SCOPE%:%VALUE_VIEWMODE%:%VALUE_ALERTMODE%]"
    %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sadm:res.view.%VALUE_RESTYPE% [%VALUE_RESLABEL%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd resview -l:%SET_LOGFILE% -trap:no
