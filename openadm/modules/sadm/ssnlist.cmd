...
@rem
@rem Usage:
@rem   <fs.InstallDir>\sadm session -cmd:list
@rem
@rem Disk locations:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\ssnlist-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=ssnlist-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  :MAIN
    ...
    set event.message="ssnlist: listing OpenADM CMDB sessions on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sadm:ssn.list [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    set event.message="ssnlist: OpenADM CMDB sessions listed. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sadm:ssn.list [%SET_SSNSEQ%]" /d !event.message! > nul
