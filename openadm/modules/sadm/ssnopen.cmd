...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Creates/Opens a new local session for OpenADM Operations.
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.InstallDir>\sadm session -cmd:open <session-id>
@rem
@rem Disk locations:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\ssnopen-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=ssnopen-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  :MAIN
    ...
    set event.message="ssnopen: opening OpenADM session on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sadm:ssn.open [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if exist %fs.CmdbDir%\%SET_SESSIONID%.open (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%

      %sys.ColorRed%
      @echo # WARNING: session already opened.
      %sys.ColorNormal%

      set event.message="ssnopen: session already opened. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "sadm:ssn.open [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if exist %fs.CmdbDir%\%SET_SESSIONID% (
      ren %fs.CmdbDir%\%SET_SESSIONID% %fs.CmdbDir%\%SET_SESSIONID%.open  >  nul
    ) else (
      md %fs.CmdbDir%\%SET_SESSIONID%.open                                >  nul
    )
    ...
    :HELP
      ...
      @echo.
      @echo   sadm session -cmd:open [session-id]
      @echo.
    ...
    :EXIT
      ...
      set event.message="ssnopen: OpenADM session opened on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sadm:ssn.open [%SET_SESSIONID%] [%SET_SSNSEQ%]" /d !event.message! > nul
