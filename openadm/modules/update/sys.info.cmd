...
@rem Date:        2009/01/18
@rem Script name: update sys.info
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Creates an updated system information summary to be used by the Alert Subsystem.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   systeminfo.exe  - win2k3 server
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\update sys.info
@rem
@rem Disk locations:
...
setlocal enabledelayedexpansion
  ...
  set EL_SYSTEMINFO_OK=0
  set EL_SYSTEMINFO_ERROR=1
  ...
  for %%i in (%fs.LogsDir%\sys.info-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=sys.info-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.BinDir%\systeminfo.exe               set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    set event.message="update sys.info: updating system information summary. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "update:sys.info [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\systeminfo > %fs.TmpDir%\sys.info.log

    if %errorlevel% NEQ %EL_SYSTEMINFO_OK% (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      set event.message="update sys.info: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "update:sys.info [%SET_SSNSEQ%]" /d !event.message! > nul
    )

    type %fs.TmpDir%\sys.info.log
    ...
    set event.message="update sys.info: system information summary updated. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "update:sys.info [%SET_SSNSEQ%]" /d !event.message! > nul
