...
@rem   alert.cmd       - openadm [update.swdb alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\update swdb {none|<server[:port]>}
...
@rem
@rem Disk locations:
@rem   fs.InstallDir: <fs.SystemDrive>\openadm\modules\update
...
@rem   fs.LogsDir:    <fs.DataDrive>\logs\openadm\update
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
@rem   update swdb none none -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : swdb.ini
@rem     - <fs.InstallDir> : swdb.[group-label].source.ini
@rem     - <fs.InstallDir> : swdb.[group-label].connection.ini
...
@rem
@rem   update swdb creates a <swdb.SessionLog> directory inside <destination-dir>.
@rem
@rem   It also needs write acces on <destination-dir>:
@rem
@rem   update swdb downloads every file on the remote web resource. It was designed
@rem   to be used in conjuntion with the  directory browsing feature present on
@rem   the web server. To perform a clean download, update swdb clears up any HTML
...
setlocal enabledelayedexpansion
  ...
  set fs.InstallDir=%fs.SystemDrive%\openadm\modules\update
  ...
  set fs.LogsDir=%fs.DataDrive%\logs\openadm\update
  ...
  if /i %VALUE_SERVERSOURCE:"=% NEQ %VALUE_TARGET_NONE% (
    ...
    if /i "%SET_PROXYMODE%" EQU "%STATUS_YES%"          set input.ValueIsOk=%STATUS_ON%
    if /i "%SET_PROXYMODE%" EQU "%STATUS_NO%"           set input.ValueIsOk=%STATUS_ON%
  ...
  for %%i in (%fs.LogsDir%\swdb-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=swdb-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log


  set input.ValueIsOk=%STATUS_ON%

  if not exist %fs.ConfDir%\el.iddb.ini               set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\swdb.ini               set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if not exist %fs.InstallDir%\swdb.%VALUE_LABELID%.source.ini           set input.ValueIsOk=%STATUS_OFF%
    if not exist %fs.InstallDir%\swdb.%VALUE_LABELID%.connection.ini       set input.ValueIsOk=%STATUS_OFF%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   %fs.InstallDir%\swdb.%VALUE_LABELID%.source.ini
      @echo #   %fs.InstallDir%\swdb.%VALUE_LABELID%.connection.ini
      @echo #
      @echo # aborting program.
      @echo.
  ...
  :MAIN
    ...
    if /i %context.SkipExec% EQU %STATUS_OFF% (
      set swdb.SessionSSL=--sslprotocol=3
      set swdb.SessionSourceFile=%fs.InstallDir%\swdb.%VALUE_LABELID%.source.ini
      set swdb.SessionConnectionFile=%fs.InstallDir%\swdb.%VALUE_LABELID%.connection.ini
      set swdb.SessionDestinationDir=!fs.DestinationDir!
      set swdb.SessionServerUser=
      set swdb.SessionServerPassword=
      set swdb.SessionProxyUser=
      set swdb.SessionProxyPassword=

      for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (!swdb.SessionConnectionFile!) do (
        if /i "%%i" EQU "server.user" set swdb.SessionServerUser=%%j
        if /i "%%i" EQU "server.pwd"  set swdb.SessionServerPassword=%%j
        if /i "%%i" EQU "proxy.user"  set swdb.SessionProxyUser=%%j
        if /i "%%i" EQU "proxy.pwd"   set swdb.SessionProxyPassword=%%j
      )

      if not exist "!swdb.SessionDestinationDir!\logs" md "!swdb.SessionDestinationDir!\logs"  >  nul
      set swdb.SessionLog="!swdb.SessionDestinationDir!\logs\%SET_LOGFILE%"

      set swdb.SessionParameters=

      for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\swdb.ini) do (
        set swdb.SessionParameters=!swdb.SessionParameters! %%i
        if /i %%i EQU !swdb.SessionSSL! set swdb.SessionIsSSL=%STATUS_YES%
      )

      set swdb.SessionPrintableParameters=-a !swdb.SessionLog! !swdb.SessionParameters!
      set swdb.SessionParameters=-a !swdb.SessionLog! !swdb.SessionParameters!

      if !swdb.SessionServerUser! NEQ anonymous   set swdb.SessionParameters=!swdb.SessionParameters! --http-user=!swdb.SessionServerUser! --http-password=!swdb.SessionServerPassword!
      if %VALUE_PROXYMODE%      EQU %STATUS_YES%  set swdb.SessionParameters=!swdb.SessionParameters! --proxy --proxy-user=!swdb.SessionProxyUser! --proxy-password=!swdb.SessionProxyPassword!

      if !swdb.SessionIsSSL! EQU %STATUS_YES% (
        set swdb.SessionProtocol=https
      ) else (
        set swdb.SessionProtocol=http
      )
    )


    set event.message="update swdb: [%VALUE_LABELID%] updating "!swdb.SessionDestinationDir!". [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "update:swdb [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\swdb.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\swdb.*.ini
    ...
    @echo # updating !swdb.SessionDestinationDir! ...
    @echo.

    set user.CurrentDir="%CD%"
    cd /d "!swdb.SessionDestinationDir!"               >  nul

    for /f "eol=# tokens=*" %%i in (!swdb.SessionSourceFile!) do (
      @echo   + repository resource: %%i
      @echo.

      %sys.ColorDark%

      %fs.BinDir%\wget !swdb.SessionParameters! %swdb.SessionProtocol%://%VALUE_SERVERSOURCE%/%%i
      ...
      if {!errorlevel!} NEQ {%EL_WGET_OK%} (
        ...
        @echo #
        @echo #   target repository resource: %%i
        @echo #   session parameters:         !swdb.SessionPrintableParameters!
        @echo #   proxy mode:                 %VALUE_PROXYMODE%
        @echo #   SSL mode:                   !swdb.SessionIsSSL!
        @echo #   web resource:               %swdb.SessionProtocol%://%VALUE_SERVERSOURCE%/%%i
        @echo.

        %sys.ColorNormal%

        set event.message="update swdb: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "update:swdb [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    ...
    if exist "!swdb.SessionLog!" (
      ...
      type "!swdb.SessionLog!"
      @echo.

      %sys.ColorNormal%
    )

    del /f /s /q !swdb.SessionDestinationDir!\*.htm?  2>&1
    ...
    :HELP
      ...
      @echo.
      @echo   update swdb [{none^|[server[:port]}]
      ...
      @echo.
      @echo     update swdb none none -id:{list^|xlist}
      @echo.
      @echo   ----------------------------------------------------------------
      @echo.
    ...
    :EXIT
      ...
      set event.message="update swdb: [%VALUE_LABELID%] "!swdb.SessionDestinationDir!" updated. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "update:swdb [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd update.swdb -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:yes
