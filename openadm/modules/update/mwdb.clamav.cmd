...
@rem Date:        2009/01/18
@rem Script name: update mwdb.clamav
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Updates ClamAV malware definition files.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   freshclam.exe   - clamwin.net
...
@rem   alert.cmd       - openadm [update.mwdb.clamav alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\update mwdb.clamav <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem     - <fs.InstallDir> : mwdb.clamav.ini
@rem     - <fs.InstallDir> : mwdb.clamav.settings.ini
...
setlocal enabledelayedexpansion
  ...
  set EL_FRESHCLAM_OK=0
  set EL_FRESHCLAM_ERROR=1
  ...
  for %%i in (%fs.LogsDir%\mwdb.clamav-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=mwdb.clamav-%yy%%mm%%dd%-%fs.LogFileCount%.log


  set input.ValueIsOk=%STATUS_ON%

  if not exist %fs.ConfDir%\el.iddb.ini                 set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\mwdb.clamav.ini          set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\mwdb.clamav.settings.ini set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\mwdb.clamav.ini
    @echo #   %fs.InstallDir%\mwdb.clamav.settings.ini
    @echo #
  ...
  for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\mwdb.clamav.ini) do (
    if /i %%i EQU av.BinDir       set av.BinDir=%%j
    if /i %%i EQU av.LogsDir      set av.LogsDir=%%~j
    if /i %%i EQU av.DbDir        set av.DbDir=%%~j
    if /i %%i EQU av.SettingsFile set av.SettingsFile=%%j
  )
  ...
  if not exist !av.BinDir!\freshclam.exe                set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   !fs.DBBinDir!\freshclam.exe
    @echo #
    @echo # aborting program.
    @echo.
  ...
  :MAIN
    ...
    for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\mwdb.clamav.settings.ini) do (
      set av.SessionParameters=!av.SessionParameters! %%i
    )

    av.SessionSettings=--config-file="!av.BinDir!\!av.SettingsFile!"
    av.SessionLog=-l "!av.LogsDir!\update-%yy%%mm%%dd%-%fs.LogFileCount%.log"
    av.SessionOutput=--datadir="!av.DbDir!"


    set event.message="update mwdb.clamav: updating system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "update:mwdb.clamav [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    !av.BinDir!\freshclam !av.SessionSettings! !av.SessionOutput! !av.SessionParameters! !av.SessionLog!

    if %errorlevel% NEQ %EL_FRESHCLAM_OK% (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      @echo #
      @echo #   av.SessionLog:        !av.LogsDir!\update-%yy%%mm%%dd%-%fs.LogFileCount%.log
      @echo #   av.SessionOutput:     !av.DbDir!
      @echo #   av.SessionSettings:   !av.SettingsFile!
      @echo #
      @echo #   av.SessionParameters: !av.SessionParameters!
      @echo.

      %sys.ColorNormal%

      set event.message="update mwdb.clamav: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "update:mwdb.clamav [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    :HELP
      ...
      @echo.
      @echo   update mwdb.clamav [-a:{yes^|no}]
      @echo.
    ...
    :EXIT
      ...
      set event.message="update mwdb.clamav: system updated. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "update:mwdb.clamav [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd update.mwdb.clamav -l:%SET_LOGFILE% -trap:yes
