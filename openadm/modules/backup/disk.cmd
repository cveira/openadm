...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Back up files to disk and performs operations to keep the log
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\backup disk {none|sysstate|<selection-filename>}
...
@rem
@rem   fs.NtBackupLogDir:  [defined in disk.<group-label>.ini]
@rem   fs.DestinationDir:  [defined in disk.<group-label>.ini]
@rem ------------------------------------------------------------------------------------
@rem Important notes:
...
@rem   backup disk none -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : disk.<group-label>.ini
@rem     - <fs.InstallDir> : disk.<group-label>.settings.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\backup.disk-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=backup.disk-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if not exist %fs.InstallDir%\disk.%VALUE_LABELID%.ini           set input.ValueIsOk=%STATUS_OFF%
    if not exist %fs.InstallDir%\disk.%VALUE_LABELID%.settings.ini  set input.ValueIsOk=%STATUS_OFF%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   %fs.InstallDir%\disk.%VALUE_LABELID%.ini
      @echo #   %fs.InstallDir%\disk.%VALUE_LABELID%.settings.ini
      @echo #
      @echo # aborting program.
      @echo.
  ...
  :MAIN
    ...
    if /i %context.SkipExec% EQU %STATUS_OFF% (
      for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\disk.%VALUE_LABELID%.ini) do (
      ...
      for /f "eol=# tokens=*" %%i in (%fs.InstallDir%\disk.%VALUE_LABELID%.settings.ini) do (
      ...
      set ntbackup.SessionDestination="!fs.DestinationDir!\backup.disk-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.bkf"
      set ntbackup.SessionLabel=backup.disk-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%
    )


    set event.message="backup disk: [%VALUE_LABELID%] starting backup. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "backup:disk [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %context.SkipExec% EQU %STATUS_OFF% (
      ...
      @echo # destination file:   !ntbackup.SessionDestination!
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\disk.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list %fs.InstallDir%\disk.*.ini
    ...
    %fs.BinDir%\ntbackup backup !ntbackup.SessionSource! !ntbackup.SessionParameters! !ntbackup.SessionMode! /j "!ntbackup.SessionLabel!" /f !ntbackup.SessionDestination!
    ...
    if %errorlevel% NEQ %EL_NTBACKUP_OK% (
      ...
      @echo #   session.destination: !ntbackup.SessionDestination!
      @echo #   session.mode:        !ntbackup.SessionMode:/m =!
      @echo #   session.parameters:  !ntbackup.SessionParameters!
      @echo.
      ...
      set event.message="backup disk: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "backup:disk [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    if NOT exist %ntbackup.SessionLog% (
      ...
      set event.message="backup disk: [%VALUE_LABELID%] unable to find %ntbackup.SessionLog%. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "backup:disk [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ) else (
    ...
    :HELP
      ...
      @echo.
      @echo   backup disk [{none^|sysstate^|[selection-filename]}]
      ...
      @echo.
      @echo     backup disk none -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="backup disk: [%VALUE_LABELID%] backup finished. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "backup:disk [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd backup.disk -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:yes
