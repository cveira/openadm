...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Creates a report with relevant system information and stores it on the selected
@rem   directory.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   winmsd.exe      - win2k3 server
...
@rem   alert.cmd       - openadm [report.si.msd alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\report si.msd
@rem
@rem Disk locations:
@rem   fs.InstallDir: <fs.SystemDrive>\openadm\modules\report
...
@rem   fs.LogsDir:    <fs.DataDrive>\logs\openadm\report
@rem   fs.ReportDir:  [defined in si.msd.ini]
@rem ------------------------------------------------------------------------------------
@rem Important notes:
@rem   winmsd.exe is not located in c:\jobs\bin because it has too many dependencies to
@rem   be a practical approach.
@rem
@rem   WARNING: verify the output of %date% in order to the script to operate the
@rem   right way.
@rem
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : si.msd.ini
...
setlocal enabledelayedexpansion
  ...
  set fs.InstallDir=%fs.SystemDrive%\openadm\modules\report
  ...
  set fs.LogsDir=%fs.DataDrive%\logs\openadm\report
  ...
  for %%i in (%fs.LogsDir%\si.msd-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=si.msd-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log


  set input.ValueIsOk=%STATUS_ON%

  if not exist %fs.ConfDir%\el.iddb.ini         set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\si.msd.ini       set input.ValueIsOk=%STATUS_OFF%
  ...
  if not exist %systemroot%\system32\winmsd.exe         set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %systemroot%\system32\winmsd.exe
    @echo #
    @echo # aborting program.
    @echo.
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\si.msd.ini) do (
    ...
    set event.message="report si.msd: creating system report information on "%fs.ReportDir%". [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "report:si.msd [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    cd /d "%fs.ReportDir%"  >  nul

    %systemroot%\system32\winmsd.exe /nfo si.msd-%yy%%mm%%dd%-%fs.LogFileCount%.nfo

    if %errorlevel% NEQ %EL_WINMSD_OK% (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      @echo #   report file:      si.msd-%yy%%mm%%dd%-%fs.LogFileCount%.nfo
      @echo.

      %sys.ColorNormal%

      set event.message="report si.msd: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "report:si.msd [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    :EXIT
      ...
      set event.message="report si.msd: system report information created on "%fs.ReportDir%". [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "report:si.msd [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd report.si.msd -l:%SET_LOGFILE% -trap:yes
