...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Analizes current and live Event Log entries in order to detect Service errors or
...
@rem   alert.cmd                          - openadm [scan.el.scm alert module]
@rem   <fs.InstallDir>\el.scm.*.[cmd|ini] - openadm [scan el.scm extension modules]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\scan el.scm <-range:{today|yesterday|lastweek}>
@rem                             <-a:{yes|no}>
@rem                             <-d:{yes|no}>
@rem
@rem Disk locations:
...
@rem     - <fs.InstallDir> : el.scm.ini
@rem     - <fs.InstallDir> : el.scm.scm.ini
@rem     - <fs.InstallDir> : el.scm.scm.status.ini
...
setlocal enabledelayedexpansion
  ...
  if /i "%SET_TIMERANGE%"    EQU ""     goto HELP
  ...
  for %%i in (%fs.LogsDir%\el.scm-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=el.scm-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\el.scm.ini             set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\el.scm.scm.ini         set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\el.scm.scm.status.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\el.scm.ini
    @echo #   %fs.InstallDir%\el.scm.scm.ini
    @echo #   %fs.InstallDir%\el.scm.scm.status.ini
    @echo #
  ...
  :MAIN
    ...
    for /f "eol=# tokens=* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\el.scm.scm.ini) do set EvtRecord.ScmPatterns=!EvtRecord.ScmPatterns!;%%i
    set EvtRecord.ScmPatterns=^(%EvtRecord.ScmPatterns:~1%^)

    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\el.scm.scm.status.ini) do (
      if /i %%i EQU start set EvtRecord.ServiceIsStartedPatterns=!EvtRecord.ServiceIsStartedPatterns!;%%j
      if /i %%i EQU stop  set EvtRecord.ServiceIsStoppedPatterns=!EvtRecord.ServiceIsStoppedPatterns!;%%j
    )

    set EvtRecord.ServiceIsStartedPatterns=^(%EvtRecord.ServiceIsStartedPatterns:~1%^)
    set EvtRecord.ServiceIsStoppedPatterns=^(%EvtRecord.ServiceIsStoppedPatterns:~1%^)


    set event.message="scan el.scm: scanning system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "scan:el.scm [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    for /f "eol=#" %%i in (%fs.InstallDir%\el.scm.ini) do (
      call %fs.InstallDir%\%%j.cmd
    )
    ...
    if exist %fs.TmpDir%\%SET_LOGFILE:.log=.exit% (
      ...
      set event.message="scan el.scm: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "scan:el.scm [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    :HELP
      ...
      @echo.
      @echo   scan el.scm [-range:{today^|yesterday^|lastweek}]
      @echo               [-a:{yes^|no}]
      @echo               [-d:{yes^|no}]
      @echo.
    ...
    :EXIT
      ...
      set event.message="scan el.scm: system scanned. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "scan:el.scm [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd scan.el.scm -l:%SET_LOGFILE% -trap:yes
