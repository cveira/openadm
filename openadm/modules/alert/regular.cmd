...
@rem Date:        2009/01/18
@rem Script name: alert [regular log files]
@rem ------------------------------------------------------------------------------------
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.AlertLogsDir%\%VALUE_ID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=%VALUE_ID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  set fs.PatternsSource=%fs.InstallDir%\regular.master.patterns.ini
  if exist %fs.InstallDir%\%VALUE_ID%.patterns.ini  set fs.PatternsSource=%fs.InstallDir%\%VALUE_ID%.patterns.ini
  ...
  :MAIN
    ...
    set mail.subject="alert [%VALUE_ID%]: patterns detected"


    set event.message="alert [%VALUE_ID%]: analyzing %VALUE_ID% results. [%fs.AlertLogsDir%\%fs.LogFile%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "alert:%VALUE_ID% [%SET_SSNSEQ%]" /d !event.message!  >  nul
    ...
    if /i %VALUE_TARGET% EQU %VALUE_TARGET_LAST% (
      for %%i in (%fs.SourceLogsDir%\%fs.LogFileRootName%-*.log) do (
    ...
    if /i %VALUE_TARGET% NEQ %VALUE_TARGET_SEARCH% (
      if NOT exist %fs.SourceLogsDir%\%fs.file.TargetLog% (
        ...
        set event.message="alert [%VALUE_ID%]: no log files to process. [%fs.AlertLogsDir%\%fs.LogFile%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "alert:%VALUE_ID% [%SET_SSNSEQ%]" /d !event.message!  >  nul
    ...
    if /i %VALUE_TARGET% NEQ %VALUE_TARGET_SEARCH% (
      ...
      %fs.BinDir%\findstr /i /m /s /g:%fs.PatternsSource% %fs.SourceLogsDir%\%fs.LogFileRootName%-*.log  >  %fs.TmpDir%\alert.%SET_LOGFILE%
      if {!errorlevel!} EQU {%EL_FINDSTR_FOUND%} set TargetLog.PatternsFound=%STATUS_ON%
    )
    ...
    if %TargetLog.PatternsFound% EQU %STATUS_ON% (
      ...
      if /i %VALUE_TARGET% NEQ %VALUE_TARGET_SEARCH% (
        set event.message="alert [%VALUE_ID%]: patterns detected on %fs.SourceLogsDir%\%fs.file.TargetLog%. [%fs.AlertLogsDir%\%fs.LogFile%]"
      ) else (
        set event.message="alert [%VALUE_ID%]: patterns detected on %fs.SourceLogsDir%\%fs.LogFileRootName%-*.log. [%fs.AlertLogsDir%\%fs.LogFile%]"
      )
      ...
      if /i %VALUE_TARGET% NEQ %VALUE_TARGET_SEARCH% (
        set event.message="alert [%VALUE_ID%]: no patterns found on %fs.SourceLogsDir%\%fs.file.TargetLog%. [%fs.AlertLogsDir%\%fs.LogFile%]"
      ) else (
        set event.message="alert [%VALUE_ID%]: no patterns found on %fs.SourceLogsDir%\%fs.LogFileRootName%-*.log. [%fs.AlertLogsDir%\%fs.LogFile%]"
      )

      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "alert:%VALUE_ID% [%SET_SSNSEQ%]" /d !event.message!  >  nul
    )
    ...
    :EXIT
      ...
      set event.message="alert [%VALUE_ID%]: %VALUE_ID% results analyzed. [%fs.AlertLogsDir%\%fs.LogFile%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "alert:%VALUE_ID% [%SET_SSNSEQ%]" /d !event.message!  >  nul
...
:SendAlert
  ...
  setlocal
    ...
    if {!errorlevel!} EQU {%STATUS_OK%} (
      ...
      set event.message="alert [%VALUE_ID%]: mail trap has been sent. [%fs.AlertLogsDir%\%fs.LogFile%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "alert:%VALUE_ID% [%SET_SSNSEQ%]" /d !event.message! > nul
    ) else (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      set event.message="alert [%VALUE_ID%]: unable to send mail trap. [%fs.AlertLogsDir%\%fs.LogFile%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "alert:%VALUE_ID% [%SET_SSNSEQ%]" /d !event.message! > nul
    )
