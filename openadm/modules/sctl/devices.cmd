...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Starts, Stops or Restarts a given Device or Device Driver.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   devcon.exe      - win2k3 support tools
...
@rem   alert.cmd       - openadm [sctl.devices alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\sctl devices {none|<"l:dev1,dev2,[...]">|<"f:group-label">}
...
@rem   <"f:dev1,dev2">   - target objects: CSV list
@rem   <"f:group-label"> - target objects stored in a configuration file devices.<group-label>.ini
@rem   [-cmd:stop]       - stops a given device or device driver.
@rem   [-cmd:start]      - start a given device or device driver.
@rem   [-cmd:restart]    - restarts a given device or device driver.
@rem   [-cmd:query]      - queries the state of a given device or device driver.
...
@rem   sctl devices none -cmd:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : devices.<group-label>.ini
@rem     - <%CD%>          : devices.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  set fs.LogFileRootName=devices
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% set fs.LogFileRootName=devices-%VALUE_TARGET:~2%
  ...
  if not exist %fs.BinDir%\devcon.exe                   set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% (
    ...
    if exist "%CD%\devices.%VALUE_TARGET:~2%.ini"              set input.ValueIsOk=%STATUS_ON%
    if exist "%fs.InstallDir%\devices.%VALUE_TARGET:~2%.ini"   set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo #
      @echo #   first file location:  "%CD%\devices.%VALUE_TARGET:~2%.ini"
      @echo #   second file location: "%fs.InstallDir%\devices.%VALUE_TARGET:~2%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\devices.%VALUE_TARGET:~2%.ini"

    if exist "%CD%\devices.%VALUE_TARGET:~2%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\devices.%VALUE_TARGET:~2%.ini"
    )
  ...
  :MAIN
    ...
    set event.message="sctl devices: performing %VALUE_CMD% operation over given device drivers. [%fs.LogsDir%\%SET_LOGFILE%]"
    if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl devices: [%VALUE_TARGET:~2%] performing %VALUE_CMD% operation over given device drivers. [%fs.LogsDir%\%SET_LOGFILE%]"

    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sctl:devices.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_LIST% (
      @echo # current devices or device drivers:
      @echo.

      %sys.ColorDark%

      %fs.BinDir%\devcon find *
      ...
      for %%i in (%fs.InstallDir%\devices.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\devices.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\devices.*.ini"
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_QUERY% (
      @echo # current state of devices or device drivers [%VALUE_TARGET%]:
      @echo.

      %sys.ColorDark%

      if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%
        %fs.BinDir%\devcon find * | %fs.BinDir%\findstr /i /g:!fs.IniFile!
      ) else (
      ...
      if /i %input.ValuesFromFile% EQU %STATUS_ON% (
        for /f "eol=# tokens=*" %%i in (!fs.IniDir!\devices.%VALUE_TARGET:~2%.ini) do (
          %fs.BinDir%\devcon status %%i
        )
      ) else (
        call :QueryStatusFromList "%VALUE_TARGET:~2%"
      )
      ...
      if /i %input.ValuesFromFile% EQU %STATUS_ON% (
        for /f "eol=# tokens=*" %%i in (!fs.IniDir!\devices.%VALUE_TARGET:~2%.ini) do (
          %fs.BinDir%\devcon resources %%i
        )
      ) else (
        call :QueryResourcesFromList "%VALUE_TARGET:~2%"
      )
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # running [%VALUE_CMD%] command over devices or device drivers defined in [%VALUE_TARGET%]:
    @echo.

    if /i %input.ValuesFromFile% EQU %STATUS_ON% (
      for /f "eol=# tokens=*" %%i in (!fs.IniDir!\devices.%VALUE_TARGET:~2%.ini) do (
        @echo   + processing device or device driver: %%i
        @echo.

        %sys.ColorDark%

        if /i %VALUE_CMD% EQU %VALUE_CMD_STOP%  %fs.BinDir%\devcon disable %%i
        if /i %VALUE_CMD% EQU %VALUE_CMD_START% %fs.BinDir%\devcon enable  %%i
        if /i %VALUE_CMD% EQU %VALUE_CMD_RESTART% (
          %fs.BinDir%\devcon disable %%i
          %fs.BinDir%\sleep %VALUE_TIMETOWAIT%
          %fs.BinDir%\devcon enable  %%i
        )

        if %errorlevel% NEQ %EL_DEVCON_OK% (
          @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
          ...
          @echo #
          @echo #   device or device driver: %%i
          @echo.

          %sys.ColorNormal%

          set event.message="sctl devices: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl devices: [%VALUE_TARGET:~2%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"

          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "sctl:devices.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
        )
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # final state of devices or device drivers [%VALUE_TARGET%]:
    @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   sctl devices [{none^|["l:dev1,dev2,[...]"]^|["f:group-label"]}]
      ...
      @echo.
      @echo     sctl devices none -cmd:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="sctl devices: %VALUE_CMD% operation over given device drivers completed. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl devices: [%VALUE_TARGET:~2%] %VALUE_CMD% operation over given device drivers completed. [%fs.LogsDir%\%SET_LOGFILE%]"

      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sctl:devices.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd sctl.devices -l:%SET_LOGFILE% -trap:yes
...
:QueryStatusFromList
  ...
  setlocal enabledelayedexpansion
    ...
    for /f "delims=%LIST_DELIMITER% tokens=1,*" %%i in (%1) do (
      ...
      if /i "%%j" NEQ "" call :QueryStatusFromList "%%j"
      goto :EOF
    )
...
:QueryResourcesFromList
  ...
  setlocal enabledelayedexpansion
    ...
    for /f "delims=%LIST_DELIMITER% tokens=1,*" %%i in (%1) do (
      ...
      if /i "%%j" NEQ "" call :QueryResourcesFromList "%%j"
      goto :EOF
    )
