...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   starts, stops or queries the state of a given scheduled jobs on the local system.
@rem
@rem Dependencies:
...
@rem   alert.cmd       - openadm [sctl.jobs alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\sctl jobs {none|all|<"l:job1,job2,[...]">|<"f:group-label">}
...
@rem   <"f:job1,job2">   - target objects: CSV list
@rem   <"f:group-label"> - target objects stored in a configuration file jobs.<group-label>.ini
@rem   [-cmd:stop]       - stops a given scheduled jobs.
@rem   [-cmd:start]      - start a given scheduled jobs.
@rem   [-cmd:restart]    - restarts a scheduled jobs.
@rem   [-cmd:query]      - queries the state of a scheduled jobs.
...
@rem   sctl jobs none -cmd:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : jobs.excluded.ini
@rem     - <fs.InstallDir> : jobs.<group-label>.ini
@rem     - <%CD%>          : jobs.<group-label>.ini
@rem     - <fs.ConfDir>    : el.iddb.ini
@rem
@rem   <jobs.excluded.ini> holds one case-sensitive exclusion string per line.
@rem   Note that no white space is allowed in OpenADM Scheduled Job names.
...
setlocal enabledelayedexpansion
  ...
  set fs.LogFileRootName=jobs
  if /i %VALUE_TARGET%      EQU %VALUE_TARGET_ALL%  set fs.LogFileRootName=jobs-%VALUE_TARGET%
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% set fs.LogFileRootName=jobs-%VALUE_TARGET:~2%
  ...
  if not exist %fs.InstallDir%\jobs.excluded.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\jobs.excluded.ini
    @echo #
  ...
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% (
    ...
    if exist "%CD%\jobs.%VALUE_TARGET:~2%.ini"               set input.ValueIsOk=%STATUS_ON%
    if exist "%fs.InstallDir%\jobs.%VALUE_TARGET:~2%.ini"    set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo #
      @echo #   first file location:  "%CD%\jobs.%VALUE_TARGET:~2%.ini"
      @echo #   second file location: "%fs.InstallDir%\jobs.%VALUE_TARGET:~2%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\jobs.%VALUE_TARGET:~2%.ini"

    if exist "%CD%\jobs.%VALUE_TARGET:~2%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\jobs.%VALUE_TARGET:~2%.ini"
    )
  ...
  if /i "%VALUE_TARGET%" EQU "%VALUE_TARGET_ALL%" (
    if /i "%context.IsCallBack%" NEQ "%STATUS_ON%" (
      for /f %%i in ('%fs.BinDir%\schtasks /query /nh') do @echo %%i                                          >>  %fs.TmpDir%\%fs.LogFile:.log=.tmp1%
      type %fs.TmpDir%\%fs.LogFile:.log=.tmp1% | %fs.BinDir%\findstr /v /g:%fs.InstallDir%\jobs.excluded.ini  >>  %fs.TmpDir%\%fs.LogFile:.log=.tmp2%
  ...
  :MAIN
    ...
    set event.message="sctl jobs: performing %VALUE_CMD% operation over given sheduled jobs. [%fs.LogsDir%\%SET_LOGFILE%]"
    if /i %VALUE_TARGET%      EQU %VALUE_TARGET_ALL%   set event.message="sctl jobs: [%VALUE_TARGET%] performing %VALUE_CMD% operation over given sheduled jobs. [%fs.LogsDir%\%SET_LOGFILE%]"
    if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl jobs: [%VALUE_TARGET:~2%] performing %VALUE_CMD% operation over given sheduled jobs. [%fs.LogsDir%\%SET_LOGFILE%]"

    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sctl:jobs.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_QUERY% (
      @echo # current state of sheduled jobs [%VALUE_TARGET%]:
      @echo.

      %sys.ColorDark%

      if /i %input.ValuesFromFile% EQU %STATUS_ON% (
        %fs.BinDir%\schtasks /query /nh | %fs.BinDir%\findstr /g:!fs.IniFile!
      ) else (
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # running [%VALUE_CMD%] command over sheduled jobs defined in [%VALUE_TARGET%]:
    @echo.
    ...
    if /i %input.ValuesFromFile% EQU %STATUS_ON% (
      for /f "eol=# tokens=*" %%i in (!fs.IniFile!) do (
        ...
        if /i %VALUE_CMD% EQU %VALUE_CMD_STOP%  %fs.BinDir%\schtasks /end /tn "%%~i"
        if /i %VALUE_CMD% EQU %VALUE_CMD_START% %fs.BinDir%\schtasks /run /tn "%%~i"
        if /i %VALUE_CMD% EQU %VALUE_CMD_RESTART% (
          %fs.BinDir%\schtasks /end /tn "%%~i"
          %fs.BinDir%\sleep    %VALUE_TIMETOWAIT%
          %fs.BinDir%\schtasks /run /tn "%%~i"
        )
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # final state of sheduled jobs [%VALUE_TARGET%]:
    @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   sctl jobs [{none^|all^|["l:job1,job2,[...]"]^|["f:group-label"]}]
      ...
      @echo.
      @echo     sctl jobs none -cmd:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="sctl jobs: %VALUE_CMD% operation over sheduled jobs completed. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET%      EQU %VALUE_TARGET_ALL%   set event.message="sctl jobs: [%VALUE_TARGET%] %VALUE_CMD% operation over sheduled jobs completed. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl jobs: [%VALUE_TARGET:~2%] %VALUE_CMD% operation over sheduled jobs completed. [%fs.LogsDir%\%SET_LOGFILE%]"

      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sctl:jobs.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul

      if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd sctl.jobs -l:%SET_LOGFILE% -trap:yes
...
:QueryFromList
  ...
  setlocal enabledelayedexpansion
    ...
    for /f "delims=%LIST_DELIMITER% tokens=1,*" %%i in (%1) do (
      ...
      %fs.BinDir%\schtasks /query /nh | %fs.BinDir%\findstr "%%~i"
