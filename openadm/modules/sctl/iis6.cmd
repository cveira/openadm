...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Starts, Stops, Restarts or Recycles the local IIS 6 server or Application pool.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   iisreset.exe    - win2k3 server
@rem   iisapp.vbs      - win2k3 server
...
@rem   alert.cmd       - openadm [sctl.iis6 alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\sctl iis6 {none|<"l:pool1,pool2,[...]">|<"f:group-label">}
@rem                           <-cmd:{start|stop|fstop|restart|frestart|recycle|query|list|xlist}>
@rem                           <-a:{yes|no}>
@rem                           <-w:99>
@rem
@rem Disk locations:
...
@rem   <"f:pool1">       - target objects: CSV list
@rem   <"f:group-label"> - target objects stored in a configuration file iis6.<group-label>.ini
@rem   [-cmd:stop]       - stops local IIS 6 server (non forcing mode).
@rem   [-cmd:fstop]      - stops local IIS 6 server (forcing mode).
@rem   [-cmd:start]      - start local IIS 6 server.
@rem   [-cmd:restart]    - restarts local IIS 6 server (non forcing mode).
@rem   [-cmd:frestart]   - restarts local IIS 6 server (forcing mode).
@rem   [-cmd:recycle]    - recycles the specified local application pools.
@rem   [-cmd:query]      - queries the state of currently configured application pools.
...
@rem   sctl iis6 none -cmd:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : iis6.<group-label>.ini
@rem     - <%CD%>          : iis6.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  set VALUE_CMD_FSTOP=stop
  set VALUE_CMD_FRESTART=restart
  set VALUE_CMD_RECYCLE=recycle
  ...
  if /i "%SET_TARGET:"=%" NEQ "%VALUE_TARGET_NONE%" (
    ...
    if /i "%VALUE_CMD%" EQU "%VALUE_CMD_FSTOP%"                       set input.ValueIsOk=%STATUS_ON%
    if /i "%VALUE_CMD%" EQU "%VALUE_CMD_FRESTART%"                    set input.ValueIsOk=%STATUS_ON%
    if /i "%VALUE_CMD%" EQU "%VALUE_CMD_RECYCLE%"                     set input.ValueIsOk=%STATUS_ON%
  ...
  set fs.LogFileRootName=iis6
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% set fs.LogFileRootName=iis6-%VALUE_TARGET:~2%
  ...
  if not exist %fs.BinDir%\iisreset.exe                 set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\iisapp.vbs               set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.BinDir%\iisreset.exe
    @echo #   %fs.InstallDir%\iisapp.vbs
  ...
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% (
    ...
    if exist "%CD%\iis6.%VALUE_TARGET:~2%.ini"                  set input.ValueIsOk=%STATUS_ON%
    if exist "%fs.InstallDir%\iis6.%VALUE_TARGET:~2%.ini"       set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo #
      @echo #   first file location:  "%CD%\iis6.%VALUE_TARGET:~2%.ini"
      @echo #   second file location: "%fs.InstallDir%\iis6.%VALUE_TARGET:~2%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\iis6.%VALUE_TARGET:~2%.ini"

    if exist "%CD%\iis6.%VALUE_TARGET:~2%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\iis6.%VALUE_TARGET:~2%.ini"
    )
  ...
  :MAIN
    ...
    set event.message="sctl iis6: performing %VALUE_CMD% operation over the local IIS 6 server or Application Pools. [%fs.LogsDir%\%SET_LOGFILE%]"
    if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl iis6: [%VALUE_TARGET:~2%] performing %VALUE_CMD% operation over the local IIS 6 server or Application Pools. [%fs.LogsDir%\%SET_LOGFILE%]"

    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sctl:iis6.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_LIST% (
      ...
      for %%i in (%fs.InstallDir%\iis6.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\iis6.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\iis6.*.ini"
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_QUERY% (
      @echo # current status of IIS 6:
      @echo.

      %sys.ColorDark%

      %fs.BinDir%\iisreset /status
      ...
      @echo # ------------------------------------------------------------------------
      %sys.ColorNormal%
      @echo # selected application pools:
      @echo.

      %sys.ColorDark%

      if /i %input.ValuesFromFile% EQU %STATUS_ON% (
        for /f "eol=#" %%i in (!fs.IniDir!\iis6.%VALUE_TARGET:~2%.ini) do (
          %fs.InstallDir%\iisapp.vbs /a "%%~i"
        )
      ) else (
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # running [%VALUE_CMD%] command over IIS/application pools in [%VALUE_TARGET%]:
    @echo.

    %sys.ColorDark%

    if /i %VALUE_CMD% EQU %VALUE_CMD_STOP%     %fs.BinDir%\iisreset /stop    /noforce
    if /i %VALUE_CMD% EQU %VALUE_CMD_FSTOP%    %fs.BinDir%\iisreset /stop
    if /i %VALUE_CMD% EQU %VALUE_CMD_START%    %fs.BinDir%\iisreset /start   /noforce
    if /i %VALUE_CMD% EQU %VALUE_CMD_RESTART%  %fs.BinDir%\iisreset /restart /noforce
    if /i %VALUE_CMD% EQU %VALUE_CMD_FRESTART% %fs.BinDir%\iisreset /restart

    if %errorlevel% NEQ %EL_IIS_OK% (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      set event.message="sctl iis6: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl iis6: [%VALUE_TARGET:~2%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"

      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "sctl:iis6.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
    )

    %sys.ColorNormal%


    if /i %VALUE_CMD% EQU %VALUE_CMD_RECYCLE% (
      if /i %input.ValuesFromFile% EQU %STATUS_ON% (
        for /f "eol=#" %%i in (!fs.IniDir!\iis6.%VALUE_TARGET:~2%.ini) do (
          @echo   + processing application pool: %%i
          @echo.

          %sys.ColorDark%

          %fs.InstallDir%\iisapp.vbs /a "%%~i" /r

          if %errorlevel% NEQ %EL_IISAPP_OK% (
            @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
    ...
    :HELP
      ...
      @echo.
      @echo   sctl iis6 [{none^|["l:pool1,pool2,[...]"]^|["f:group-label"]}]
      @echo             [-cmd:{start^|stop^|fstop^|restart^|frestart^|recycle
      @echo                  ^|query^|list^|xlist}]
      @echo             [-a:{yes^|no}]
      @echo             [-w:99]
      @echo.
      @echo   ----------------------------------------------------------------
      ...
      @echo.
      @echo     sctl iis6 none -cmd:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="sctl iis6: %VALUE_CMD% operation over the local IIS 6 server or Application Pools completed. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl iis6: [%VALUE_TARGET:~2%] %VALUE_CMD% operation over the local IIS 6 server or Application Pools completed. [%fs.LogsDir%\%SET_LOGFILE%]"

      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sctl:iis6.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd sctl.iis6 -l:%SET_LOGFILE% -trap:yes
