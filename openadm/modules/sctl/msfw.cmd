...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Starts, Stops, Restarts the local Windows Firewall service or Enables or Disables
@rem   particular network services.
@rem
@rem Dependencies:
...
@rem   alert.cmd       - openadm [sctl.msfw alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\sctl msfw {none|<"f:group-label">}
@rem                           <-cmd:{start|stop|restart|enable|disable|query|list|xlist}>
@rem                           <-a:{yes|no}>
@rem                           <-w:99>
@rem
@rem Disk locations:
...
@rem   [-cmd:stop]       - stops the local firewall service.
@rem   [-cmd:start]      - start the local firewall service.
@rem   [-cmd:restart]    - restarts the local firewall service.
@rem   [-cmd:enable]     - enables access to the specified network services.
@rem   [-cmd:disable]    - disables access to the specified network services.
@rem   [-cmd:query]      - queries the state of currently allowed network services.
...
@rem   sctl msfw none -cmd:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : msfw.<group-label>.ini
@rem     - <%CD%>          : msfw.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  set fs.LogFileRootName=msfw
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% set fs.LogFileRootName=msfw-%VALUE_TARGET:~2%
  ...
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% (
    ...
    if exist "%CD%\msfw.%VALUE_TARGET:~2%.ini"               set input.ValueIsOk=%STATUS_ON%
    if exist "%fs.InstallDir%\msfw.%VALUE_TARGET:~2%.ini"    set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   first file location:  "%CD%\msfw.%VALUE_TARGET:~2%.ini"
      @echo #   second file location: "%fs.InstallDir%\msfw.%VALUE_TARGET:~2%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\msfw.%VALUE_TARGET:~2%.ini"

    if exist "%CD%\msfw.%VALUE_TARGET:~2%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\msfw.%VALUE_TARGET:~2%.ini"
    )
  )
  ...
  :MAIN
    ...
    set event.message="sctl msfw: performing %VALUE_CMD% operation on local firewall service. [%fs.LogsDir%\%SET_LOGFILE%]"
    if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl msfw: [%VALUE_TARGET:~2%] performing %VALUE_CMD% operation on local firewall service. [%fs.LogsDir%\%SET_LOGFILE%]"

    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sctl:msfw.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_LIST% (
      ...
      for %%i in (%fs.InstallDir%\msfw.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\msfw.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\msfw.*.ini"
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_QUERY% (
      @echo # current state of local firewall service:
      @echo.
    ...
    @echo # current state for local firewall service:
    @echo.
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current config for local firewall service:
    @echo.
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # running [%VALUE_CMD%] command on local firewall for services group %VALUE_TARGET%:
    @echo.

    %sys.ColorDark%

    set target.IsNetworkPort=%STATUS_ON%

    if /i %VALUE_CMD% EQU %VALUE_CMD_STOP%    set target.IsNetworkPort=%STATUS_OFF%
    if /i %VALUE_CMD% EQU %VALUE_CMD_START%   set target.IsNetworkPort=%STATUS_OFF%
    if /i %VALUE_CMD% EQU %VALUE_CMD_RESTART% set target.IsNetworkPort=%STATUS_OFF%

    if /i %target.IsNetworkPort% EQU %STATUS_OFF% (
      if /i %VALUE_CMD% EQU %VALUE_CMD_STOP%  netsh firewall set opmode disable
      if /i %VALUE_CMD% EQU %VALUE_CMD_START% netsh firewall set opmode enable exceptions=enable
      ...
      if !errorlevel! NEQ %EL_NETSH_OK% (
        ...
        set event.message="sctl msfw: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
        if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl msfw: [%VALUE_TARGET:~2%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"

        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "sctl:msfw.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    ) else (
      for /f "eol=# tokens=1,2,3,4,5,6 delims=%FIELD_DELIMITER%" %%i in (!fs.IniDir!\msfw.%VALUE_TARGET:~2%.ini) do (
        %sys.ColorDark%

        @echo   + processing port: %%i %%j %%k
        @echo.

        if /i %VALUE_CMD% EQU %VALUE_CMD_ENABLE%  netsh firewall set portopening %%i %%j %%k enable  %%l %%m %%n
        if /i %VALUE_CMD% EQU %VALUE_CMD_DISABLE% netsh firewall set portopening %%i %%j %%k disable %%l %%m %%n
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # final state for local firewall service:
    @echo.
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # final config for local firewall service:
    @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   sctl msfw [{none^|["f:group-label"]}]
      @echo             [-cmd:{start^|stop^|restart^|enable^|disable^|query^|list^|xlist}]
      @echo             [-a:{yes^|no}]
      @echo             [-w:99]
      @echo.
      @echo   ----------------------------------------------------------------
      ...
      @echo.
      @echo     sctl msfw none -cmd:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="sctl msfw: %VALUE_CMD% operation on local firewall service completed. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl msfw: [%VALUE_TARGET:~2%] %VALUE_CMD% operation on local firewall service completed. [%fs.LogsDir%\%SET_LOGFILE%]"

      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sctl:msfw.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd sctl.msfw -l:%SET_LOGFILE% -trap:yes
