...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   starts a group of services belonging to a certain group
@rem
@rem Dependencies:
...
@rem   alert.cmd       - openadm [sctl.services alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\sctl services {none|<"l:service1,service2,[...]">|<"f:group-label">}
...
@rem   <"f:group-label"> - target objects stored in a configuration file services.<group-label>.ini
@rem   [-cmd:stop]       - stops a given service or device driver.
@rem   [-cmd:start]      - start a given service or device driver.
@rem   [-cmd:restart]    - restarts a given service or device driver.
@rem   [-cmd:query]      - queries the state of a service or device driver.
...
@rem   sctl services none -cmd:{list|xlist}
@rem
@rem Important notes:
...
setlocal enabledelayedexpansion
  ...
  set fs.LogFileRootName=services
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% set fs.LogFileRootName=services-%VALUE_TARGET:~2%
  ...
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% (
    ...
    if exist "%CD%\services.%VALUE_TARGET:~2%.ini"               set input.ValueIsOk=%STATUS_ON%
    if exist "%fs.InstallDir%\services.%VALUE_TARGET:~2%.ini"    set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo #
      @echo #   first file location:  "%CD%\services.%VALUE_TARGET:~2%.ini"
      @echo #   second file location: "%fs.InstallDir%\services.%VALUE_TARGET:~2%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\services.%VALUE_TARGET:~2%.ini"

    if exist "%CD%\services.%VALUE_TARGET:~2%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\services.%VALUE_TARGET:~2%.ini"
    )
  ...
  :MAIN
    ...
    set event.message="sctl services: performing %VALUE_CMD% operation over given services or device drivers. [%fs.LogsDir%\%SET_LOGFILE%]"
    if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl services: [%VALUE_TARGET:~2%] performing %VALUE_CMD% operation over given services or device drivers. [%fs.LogsDir%\%SET_LOGFILE%]"

    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sctl:services.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_QUERY% (
      @echo # current state of service/driver [%VALUE_TARGET%]:
      @echo.

      %sys.ColorDark%

      if /i %input.ValuesFromFile% EQU %STATUS_ON% (
        wmic service list brief | %fs.BinDir%\findstr /i /g:!fs.IniFile!
      ) else (
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # running [%VALUE_CMD%] command over services/drivers defined in [%VALUE_TARGET%]:
    @echo.

    if /i %input.ValuesFromFile% EQU %STATUS_ON% (
      for /f "eol=# tokens=*" %%i in (!fs.IniDir!\services.%VALUE_TARGET:~2%.ini) do (
        @echo   + processing service/driver: %%i
        @echo.

        %sys.ColorDark%

        if /i %VALUE_CMD% EQU %VALUE_CMD_STOP%  %fs.BinDir%\sc stop  "%%~i"
        if /i %VALUE_CMD% EQU %VALUE_CMD_START% %fs.BinDir%\sc start "%%~i"
        if /i %VALUE_CMD% EQU %VALUE_CMD_RESTART% (
          %fs.BinDir%\sc stop  "%%~i"
          %fs.BinDir%\sleep    %VALUE_TIMETOWAIT%
          %fs.BinDir%\sc start "%%~i"
        )
        ...
        if %errorlevel% NEQ %EL_SC_OK% (
          ...
          @echo #
          @echo #   service/driver: %%i
          @echo.

          %sys.ColorNormal%

          set event.message="sctl services: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl services: [%VALUE_TARGET:~2%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"

          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "sctl:services.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
        )
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # final state of service/driver [%VALUE_TARGET%]:
    @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   sctl services [{none^|["l:service1,service2,[...]"]^|["f:group-label"]}]
      ...
      @echo.
      @echo     sctl services none -cmd:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="sctl services: %VALUE_CMD% operation over given services or device drivers completed. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl services: [%VALUE_TARGET:~2%] %VALUE_CMD% operation over given services or device drivers completed. [%fs.LogsDir%\%SET_LOGFILE%]"

      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sctl:services.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd sctl.services -l:%SET_LOGFILE% -trap:yes
...
:QueryFromList
  ...
  setlocal enabledelayedexpansion
    ...
    for /f "delims=%LIST_DELIMITER% tokens=1,*" %%i in (%1) do (
      ...
      wmic service list brief | %fs.BinDir%\findstr /i "%%~i"
