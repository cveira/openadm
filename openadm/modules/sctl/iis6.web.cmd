...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Starts, Stops or Restarts a given virtual web server
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   iisweb.vbs      - win2k3 server
...
@rem   alert.cmd       - openadm [sctl.iis6.web alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\sctl iis6.web {none|all|<"l:vserver1,vserver2,[...]">|<"f:group-label">}
...
@rem   <"f:vserver1">    - target objects: CSV list
@rem   <"f:group-label"> - target objects stored in a configuration file iis6.web.<group-label>.ini
@rem   [-cmd:stop]       - stops a given virtual server.
@rem   [-cmd:start]      - start a given virtual server.
@rem   [-cmd:restart]    - stops a given virtual server.
@rem   [-cmd:query]      - queries the state of a a given virtual server.
...
@rem   sctl iis6.web none -cmd:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : iis6.web.excluded.ini
@rem     - <fs.InstallDir> : iis6.web.<group-label>.ini
@rem     - <%CD%>          : iis6.web.<group-label>.ini
@rem     - <fs.ConfDir>    : el.iddb.ini
@rem
@rem   <iis6.web.excluded.ini> holds one case-sensitive exclusion string per line.
@rem   Note that no white space is allowed in OpenADM Virtual Server names.
@rem   Exclusion only takes place when VALUE_TARGET_ALL is specified.
@rem
...
setlocal enabledelayedexpansion
  ...
  set fs.LogFileRootName=iis6.web
  if /i %VALUE_TARGET%      EQU %VALUE_TARGET_ALL%  set fs.LogFileRootName=iis6.web-%VALUE_TARGET%
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% set fs.LogFileRootName=iis6.web-%VALUE_TARGET:~2%
  ...
  if not exist %fs.InstallDir%\iis6.web.excluded.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\iis6.web.excluded.ini
    @echo #
  ...
  if not exist %fs.InstallDir%\iisweb.vbs               set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE% (
    ...
    if exist "%CD%\iis6.web.%VALUE_TARGET:~2%.ini"              set input.ValueIsOk=%STATUS_ON%
    if exist "%fs.InstallDir%\iis6.web.%VALUE_TARGET:~2%.ini"   set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo #
      @echo #   first file location:  "%CD%\iis6.web.%VALUE_TARGET:~2%.ini"
      @echo #   second file location: "%fs.InstallDir%\iis6.web.%VALUE_TARGET:~2%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\iis6.web.%VALUE_TARGET:~2%.ini"

    if exist "%CD%\iis6.web.%VALUE_TARGET:~2%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\iis6.web.%VALUE_TARGET:~2%.ini"
    )
  ...
  if /i "%VALUE_TARGET%" EQU "%VALUE_TARGET_ALL%" (
    if /i "%context.IsCallBack%" NEQ "%STATUS_ON%" (
      for /f "delims=(" %%i in ('%fs.InstallDir%\iisweb.vbs /query') do @echo %%i                                 >>  %fs.TmpDir%\%fs.LogFile:.log=.tmp1%
      type %fs.TmpDir%\%fs.LogFile:.log=.tmp1% | %fs.BinDir%\findstr /v /g:%fs.InstallDir%\iis6.web.excluded.ini  >>  %fs.TmpDir%\%fs.LogFile:.log=.tmp2%
  ...
  :MAIN
    ...
    set event.message="sctl iis6.web: performing %VALUE_CMD% operation over virtual web server. [%fs.LogsDir%\%SET_LOGFILE%]"
    if /i %VALUE_TARGET%      EQU %VALUE_TARGET_ALL%   set event.message="sctl iis6.web: [%VALUE_TARGET%] performing %VALUE_CMD% operation over virtual web server. [%fs.LogsDir%\%SET_LOGFILE%]"
    if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl iis6.web: [%VALUE_TARGET:~2%] performing %VALUE_CMD% operation over virtual web server. [%fs.LogsDir%\%SET_LOGFILE%]"

    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "sctl:iis6.web.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_LIST% (
      ...
      for %%i in (%fs.InstallDir%\iis6.web.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\iis6.web.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\iis6.web.*.ini"
    ...
    if /i %VALUE_CMD% EQU %VALUE_CMD_QUERY% (
      @echo # current state of local virtual web servers [%VALUE_TARGET%]:
      @echo.

      %sys.ColorDark%

      if /i %input.ValuesFromFile% EQU %STATUS_ON% (
        %fs.InstallDir%\iisweb.vbs /query | %fs.BinDir%\findstr /g:!fs.IniFile!
      ) else (
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # running [%VALUE_CMD%] command over virtual web servers in [%VALUE_TARGET%]:
    @echo.

    if /i %input.ValuesFromFile% EQU %STATUS_ON% (
      for /f "eol=# tokens=*" %%i in (!fs.IniFile!) do (
        @echo   + processing virtual web server: %%i
        @echo.

        %sys.ColorDark%

        if /i %VALUE_CMD% EQU %VALUE_CMD_STOP%  %fs.InstallDir%\iisweb.vbs /stop  "%%~i"
        if /i %VALUE_CMD% EQU %VALUE_CMD_START% %fs.InstallDir%\iisweb.vbs /start "%%~i"
        if /i %VALUE_CMD% EQU %VALUE_CMD_RESTART% (
          %fs.InstallDir%\iisweb.vbs /stop  "%%~i"
          %fs.BinDir%\sleep %VALUE_TIMETOWAIT%
          %fs.InstallDir%\iisweb.vbs /start "%%~i"
        )

        if %errorlevel% NEQ %EL_IISWEB_OK% (
          @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
          ...
          @echo #
          @echo #   virtual web server: %%i
          @echo.

          %sys.ColorNormal%

          set event.message="sctl iis6.web: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          if /i %VALUE_TARGET%      EQU %VALUE_TARGET_ALL%   set event.message="sctl iis6.web: [%VALUE_TARGET%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl iis6.web: [%VALUE_TARGET:~2%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"

          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "sctl:iis6.web.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul
        )
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # final state of local virtual web servers [%VALUE_TARGET%]:
    @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   sctl iis6.web [{none^|all^|["l:vserver1,vserver2,[...]"]^|["f:group-label"]}]
      ...
      @echo.
      @echo     sctl iis6.web none -cmd:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="sctl iis6.web: %VALUE_CMD% operation over virtual web server completed. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET%      EQU %VALUE_TARGET_ALL%   set event.message="sctl iis6.web: [%VALUE_TARGET%] %VALUE_CMD% operation over virtual web server completed. [%fs.LogsDir%\%SET_LOGFILE%]"
      if /i %VALUE_TARGET:~0,2% EQU %VALUE_TARGET_FILE%  set event.message="sctl iis6.web: [%VALUE_TARGET:~2%] %VALUE_CMD% operation over virtual web server completed. [%fs.LogsDir%\%SET_LOGFILE%]"

      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "sctl:iis6.web.%VALUE_CMD% [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd sctl.iis6.web -l:%SET_LOGFILE% -trap:yes
...
:QueryFromList
  ...
  setlocal enabledelayedexpansion
    ...
    for /f "delims=%LIST_DELIMITER% tokens=1,*" %%i in (%1) do (
      ...
      %fs.InstallDir%\iisweb.vbs /query | %fs.BinDir%\findstr "%%~i"
