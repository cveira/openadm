...
@rem Date:        2009/01/18
@rem Script name: baseline locale
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines locale settings.
@rem
@rem Dependencies:
@rem   nlsinfo.exe     - win2k3 reskit
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline locale <-a:{yes|no}>
@rem                                 <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in locale.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\locale-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=locale-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\locale.ini       set input.ValueIsOk=%STATUS_OFF%
  ...
  if not exist %fs.BinDir%\nlsinfo.exe                  set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\locale.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.locale
    ...
    set event.message="baseline locale: baselining locale settings. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\nlsinfo  >  %fs.CmdbDir%\%baseline.OutputTmpFile%

    if %errorlevel% NEQ %EL_NLSINFO_OK% (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline locale: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline locale: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline locale: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline locale: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if %errorlevel% NEQ %EL_NLSINFO_OK% goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline locale: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :EXIT
      ...
      set event.message="baseline locale: locale settings baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline locale] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline locale: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline locale: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:locale [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.locale -l:%SET_LOGFILE% -trap:yes
