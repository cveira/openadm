...
<xsl:template match="PROPERTY" xml:space="preserve">
  <property>
    <name><xsl:value-of select="@NAME"/></name><value><xsl:apply-templates select="VALUE"><xsl:with-param name="type"><xsl:value-of select="@TYPE"/></xsl:with-param></xsl:apply-templates></value>
  </property>
</xsl:template>

<xsl:template match="PROPERTY.ARRAY" xml:space="preserve">
  <property>
    <name><xsl:value-of select="@NAME"/></name><value><xsl:apply-templates select="VALUE.ARRAY"><xsl:with-param name="includequotes">true</xsl:with-param><xsl:with-param name="type"><xsl:value-of select="@TYPE"/></xsl:with-param></xsl:apply-templates></value>
  </property>
</xsl:template>

<xsl:template match="PROPERTY.REFERENCE" xml:space="preserve">
  <property>
    <name><xsl:value-of select="@NAME"/></name><xsl:apply-templates select="VALUE.REFERENCE"></xsl:apply-templates>
  </property>
</xsl:template>

<xsl:template match="VALUE.REFERENCE"><value><xsl:apply-templates select="INSTANCEPATH/NAMESPACEPATH"/><xsl:apply-templates select="INSTANCEPATH/INSTANCENAME|INSTANCENAME"/></value></xsl:template>
