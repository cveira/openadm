...
@rem Date:        2009/01/18
@rem Script name: baseline ipports
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines the local TCP/UDP server ports.
@rem
@rem Dependencies:
@rem   portqry.exe     - microsoft.com (v2.0)
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline ipports <-a:{yes|no}>
@rem                                  <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in ipports.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\ipports-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=ipports-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\ipports.ini      set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\ipports.ini
    @echo #
  ...
  if not exist %fs.BinDir%\portqry.exe                  set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\ipports.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.ipports
    ...
    set event.message="baseline ipports: baselining IP server ports. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    set portqry.FailCode=%EL_PORTQRY_OK%
    %fs.BinDir%\portqry -local | %fs.BinDir%\logparser -q:on -iw:off -stats:off "SELECT DISTINCT SUBSTR(EXTRACT_TOKEN(Text,0, ' '), SUB(STRLEN(EXTRACT_TOKEN(Text, 0, ' ')),3)) AS Protocol, TO_INT(EXTRACT_TOKEN(Text, 1, ' ')) AS Port FROM stdin WHERE Port > 0 ORDER BY Protocol, Port" > %fs.CmdbDir%\%baseline.OutputTmpFile%
    if {!errorlevel!} NEQ {%EL_PORTQRY_OK%} set portqry.FailCode=%EL_PORTQRY_ERROR%

    @echo.
    %sys.ColorNormal%

    if {!portqry.FailCode!} NEQ {%EL_PORTQRY_OK%} (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline ipports: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline ipports: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline ipports: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline ipports: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\portqry -local | %fs.BinDir%\logparser -q:on -iw:off -stats:off "SELECT EXTRACT_TOKEN(TRIM(EXTRACT_TOKEN(Text,1,':')),0, ' ') AS Pid, SUBSTR(EXTRACT_TOKEN(TRIM(EXTRACT_TOKEN(Text,1,':')),1, ' '),1,SUB(STRLEN(EXTRACT_TOKEN(TRIM(EXTRACT_TOKEN(Text,1,':')),1, ' ')),2)) AS Process FROM stdin WHERE Text LIKE 'Process ID%%'"

    if %errorlevel% NEQ %EL_PORTQRY_OK% goto ERROR

    @echo.
    @echo # current sessions  ////////////////////////////////////////////////////
    @echo.

    %fs.BinDir%\portqry -local | %fs.BinDir%\logparser -q:on -iw:off -stats:off "SELECT SUBSTR(EXTRACT_TOKEN(Text,0, ' '), SUB(STRLEN(EXTRACT_TOKEN(Text, 0, ' ')),3)) AS Protocol, TO_INT(EXTRACT_TOKEN(Text, 1, ' ')) AS Port, DIV(COUNT(*),2) FROM stdin WHERE Port > 0 GROUP BY Port, Protocol ORDER BY Protocol, Port"

    if %errorlevel% NEQ %EL_PORTQRY_OK% goto ERROR

    @echo.
    @echo # IP ports per processes  //////////////////////////////////////////////
    @echo.

    %fs.BinDir%\portqry -local | %fs.BinDir%\logparser -q:on -iw:off -stats:off "SELECT DISTINCT TO_INT(EXTRACT_TOKEN(REPLACE_STR(EXTRACT_TOKEN(Text,0, ' '), HEX_TO_PRINT('09'), ' '),0 , ' ')) AS Pid, TO_INT(EXTRACT_TOKEN(Text, 1, ' ')) AS Port FROM stdin WHERE Port > 0 ORDER BY Pid, Port"

    if %errorlevel% NEQ %EL_PORTQRY_OK% goto ERROR

    @echo.
    @echo # sessions per processes  //////////////////////////////////////////////
    @echo.

    %fs.BinDir%\portqry -local | %fs.BinDir%\logparser -q:on -iw:off -stats:off "SELECT DISTINCT TO_INT(EXTRACT_TOKEN(REPLACE_STR(EXTRACT_TOKEN(Text,0, ' '), HEX_TO_PRINT('09'), ' '),0 , ' ')) AS Pid, DIV(COUNT(*),2) AS Total, MUL(PROPCOUNT(*),100) AS Percentage FROM stdin WHERE  TO_INT(EXTRACT_TOKEN(Text, 1, ' ')) > 0 GROUP BY Pid ORDER BY Total DESC"
    ...
    :ERROR
      ...
      set event.message="baseline ipports: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :HELP
      ...
      @echo.
      @echo   baseline ipports [-a:{yes^|no}]
      @echo                    [-sr:{yes^|no}]
      @echo.
    ...
    :EXIT
      ...
      set event.message="baseline ipports: IP server ports baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline ipports] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline ipports: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline ipports: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:ipports [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.ipports -l:%SET_LOGFILE% -trap:yes
