...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines services exposed through MSRPC.
@rem
@rem Dependencies:
@rem   rpcdump.exe     - win2k3 reskit
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline rpc <-a:{yes|no}>
@rem                              <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in rpc.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\rpc-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=rpc-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\rpc.ini          set input.ValueIsOk=%STATUS_OFF%
  ...
  if not exist %fs.BinDir%\rpcdump.exe                  set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\rpc.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.rpc
    ...
    set event.message="baseline rpc: baselining services exposed through MSRPC. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\rpcdump /s localhost        >>  %fs.CmdbDir%\%baseline.OutputTmpFile%
    @echo.                                  >>  %fs.CmdbDir%\%baseline.OutputTmpFile%
    %fs.BinDir%\rpcdump /s localhost /v /i  >>  %fs.CmdbDir%\%baseline.OutputTmpFile%

   if {!errorlevel!} NEQ {%EL_RCPDUMP_OK%} (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline rpc: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline rpc: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline rpc: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline rpc: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current services exposed through MSRPC:
    @echo.

    %sys.ColorDark%

    %fs.BinDir%\rpcdump /s localhost
    @echo.
    %fs.BinDir%\rpcdump /s localhost /v /i

    if {!errorlevel!} NEQ {%EL_RCPDUMP_OK%} goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline rpc: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :EXIT
      ...
      set event.message="baseline rpc: services exposed through MSRPC baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline rpc] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline rpc: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline rpc: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:rpc [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.rpc -l:%SET_LOGFILE% -trap:yes
