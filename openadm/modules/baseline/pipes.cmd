...
@rem Date:        2009/01/18
@rem Script name: baseline pipes
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines the contents of the Named Pipes File System (NPFS).
@rem
@rem Dependencies:
@rem   pipelist.exe    - www.sysinternals.com (v1.01)
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline pipes <-a:{yes|no}>
@rem                                <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in pipes.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
@rem     - <fs.InstallDir> : pipes.ini
@rem     - <fs.InstallDir> : pipes.excluded.ini
...
setlocal enabledelayedexpansion
  ...
  set EL_PIPELIST_OK=0
  set EL_PIPELIST_ERROR=1
  ...
  for %%i in (%fs.LogsDir%\pipes-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=pipes-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\pipes.ini           set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\pipes.excluded.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\pipes.ini
    @echo #   %fs.InstallDir%\pipes.excluded.ini
    @echo #
  ...
  if not exist %fs.BinDir%\pipelist.exe                 set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\pipes.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.pipes
    ...
    set event.message="baseline pipes: baselining system named pipes. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    set pipelist.FailCode=%EL_PIPELIST_OK%
    %fs.BinDir%\pipelist | %fs.BinDir%\findstr /v /g:%fs.InstallDir%\pipes.excluded.ini | %fs.BinDir%\logparser -q:on -iw:off -stats:off "SELECT EXTRACT_TOKEN(REPLACE_STR(EXTRACT_TOKEN(Text,0, ' '), HEX_TO_PRINT('09'), ' '),0 , ' ') AS Pipe FROM stdin WHERE Pipe IS NOT NULL ORDER BY Pipe" > %fs.CmdbDir%\%baseline.OutputTmpFile%
    if {!errorlevel!} NEQ {%EL_PIPELIST_OK%} set pipelist.FailCode=%EL_PIPELIST_ERROR%

    @echo.
    %sys.ColorNormal%

    if {!pipelist.FailCode!} NEQ {%EL_PIPELIST_OK%} (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline pipes: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline pipes: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%

        set user.ExitCode=%EL_STATUS_ERROR%
        ...
        set event.message="baseline pipes: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline pipes: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current named pipes:
    @echo.

    %sys.ColorDark%

    %fs.BinDir%\pipelist | %fs.BinDir%\findstr /v /g:%fs.InstallDir%\pipes.excluded.ini | %fs.BinDir%\logparser -q:on -iw:off -stats:off "SELECT EXTRACT_TOKEN(REPLACE_STR(EXTRACT_TOKEN(Text,0, ' '), HEX_TO_PRINT('09'), ' '),0 , ' ') AS Pipe FROM stdin WHERE Pipe IS NOT NULL ORDER BY Pipe"

    if {!errorlevel!} NEQ {%EL_PIPELIST_OK%} goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline pipes: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :EXIT
      ...
      set event.message="baseline pipes: system named pipes baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline pipes] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline pipes: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline pipes: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:pipes [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.pipes -l:%SET_LOGFILE% -trap:yes
