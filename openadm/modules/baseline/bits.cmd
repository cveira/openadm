...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines bits settings.
@rem
@rem Dependencies:
@rem   bitsadmin.exe   - win2k3 reskit
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline bits <-a:{yes|no}>
@rem                               <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in bits.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  set EL_BITSADMIN_OK=0
  set EL_BITSADMIN_ERROR=1
  ...
  for %%i in (%fs.LogsDir%\bits-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=bits-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\bits.ini         set input.ValueIsOk=%STATUS_OFF%
  ...
  if not exist %fs.BinDir%\bitsadmin.exe                set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\bits.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.bits
    ...
    set event.message="baseline bits: baselining BITS settings. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\bitsadmin /list /allusers /verbose  >  %fs.CmdbDir%\%baseline.OutputTmpFile%

    if %errorlevel% NEQ %EL_BITSADMIN_OK% (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline bits: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline bits: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline bits: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline bits: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current BITS settings:
    @echo.

    %sys.ColorDark%

    %fs.BinDir%\bitsadmin /list /allusers /verbose

    if %errorlevel% NEQ %EL_BITSADMIN_OK% goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline bits: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :EXIT
      ...
      set event.message="baseline bits: BITS settings baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline bits] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline bits: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline bits: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:bits [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.bits -l:%SET_LOGFILE% -trap:yes
