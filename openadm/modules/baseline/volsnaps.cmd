...
@rem Date:        2009/01/18
@rem Script name: baseline volsnaps
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines the auto-run system entry-points.
@rem
@rem Dependencies:
@rem   vssadmin.exe    - win2k3 server
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline volsnaps <-a:{yes|no}>
@rem                                   <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in volsnaps.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
@rem     - <fs.InstallDir> : volsnaps.ini
...
setlocal enabledelayedexpansion
  ...
  set EL_VSSADMIN_OK=0
  set EL_VSSADMIN_ERROR=1
  ...
  for %%i in (%fs.LogsDir%\volsnaps-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=volsnaps-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\volsnaps.ini     set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\volsnaps.ini
    @echo #
  ...
  if not exist %fs.BinDir%\vssadmin.exe                 set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\volsnaps.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.volsnaps
    ...
    set event.message="baseline volsnaps: baselining VSS snapshots on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\vssadmin list shadows    >>  %fs.CmdbDir%\%baseline.OutputTmpFile%
    @echo.                               >>  %fs.CmdbDir%\%baseline.OutputTmpFile%
    %fs.BinDir%\vssadmin list providers  >>  %fs.CmdbDir%\%baseline.OutputTmpFile%
    @echo.                               >>  %fs.CmdbDir%\%baseline.OutputTmpFile%
    %fs.BinDir%\vssadmin list writers    >>  %fs.CmdbDir%\%baseline.OutputTmpFile%

    if {!errorlevel!} NEQ {%EL_VSSADMIN_OK%} (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline volsnaps: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline volsnaps: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline volsnaps: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline volsnaps: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current VSS snapshots on local system:
    @echo.

    %sys.ColorDark%

    %fs.BinDir%\vssadmin list shadows
    @echo.
    %fs.BinDir%\vssadmin list providers
    @echo.
    %fs.BinDir%\vssadmin list writers

    if {!errorlevel!} NEQ {%EL_VSSADMIN_OK%} goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline volsnaps: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :HELP
      ...
      @echo.
      @echo   baseline volsnaps [-a:{yes^|no}]
      @echo                    [-sr:{yes^|no}]
      @echo.
    ...
    :EXIT
      ...
      set event.message="baseline volsnaps: VSS snapshots on local system baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline volsnaps] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline volsnaps: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline volsnaps: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:volsnaps [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.volsnaps -l:%SET_LOGFILE% -trap:yes
