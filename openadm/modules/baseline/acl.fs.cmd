...
@rem Date:        2009/01/18
@rem Script name: baseline acl.fs
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines File Systems ACLs.
@rem
@rem Dependencies:
@rem   dump.cmd        - openadm [acl.fs baseline module]
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline acl.fs <-a:{yes|no}>
@rem                                 <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in el.fs.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
@rem     - <fs.InstallDir>         : acl.fs.ini
@rem     - <fs.InstallDir>\..\dump : acl.fs.baseline.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\acl.fs-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=acl.fs-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\acl.fs.ini                   set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\..\dump\acl.fs.baseline.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\acl.fs.ini
    @echo #   %fs.InstallDir%\..\dump\acl.fs.baseline.ini
    @echo #
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\acl.fs.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.acl.fs
    ...
    set event.message="baseline acl.fs: baselining file systems ACLs. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    call %fs.SysOpDir%\dump acl.fs %fs.DumpDir% -id:baseline -a:no
    ...
    if {!errorlevel!} NEQ {%EL_DUMP_OK%} (
      ...
      for /f "usebackq tokens=1,*" %%i in (`%fs.BinDir%\logparser -i:fs -q:on -iw:off -stats:off -headers:off -o:nat "SELECT TOP 1 Name, LastWriteTime FROM %fs.DumpDir%\acl.fs-baseline-%yy%%mm%%dd%-*.* ORDER BY LastWriteTime DESC"`) do set baseline.SourceFiles=%%i
      for /f "delims=- tokens=1-5" %%a in ("!baseline.SourceFiles!") do (
  	    setlocal
    	    for %%j in (%fs.DumpDir%\%%a-%%b-%%c-%%d-*.*) do (
      			@echo   + including file: %fs.DumpDir%\%%~nxj
      			@type %fs.DumpDir%\%%~nxj  >>  %fs.CmdbDir%\%baseline.OutputTmpFile%
      		)
        endlocal
      )
    )
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline acl.fs: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline acl.fs: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline acl.fs: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline acl.fs: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current file systems ACLs:
    @echo.
    ...
    :ERROR
      ...
      set event.message="baseline acl.fs: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :EXIT
      ...
      set event.message="baseline acl.fs: file systems ACLs baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline acl.fs] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline acl.fs: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline acl.fs: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:acl.fs [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.acl.fs -l:%SET_LOGFILE% -trap:yes
