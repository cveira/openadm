...
@rem Date:        2009/01/18
@rem Script name: baseline sysmon
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines System Monitor Services declared on local system.
@rem
@rem Dependencies:
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline sysmon <-a:{yes|no}>
@rem                                 <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in sysmon.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\sysmon-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=sysmon-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\sysmon.ini       set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\sysmon.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.sysmon
    ...
    set event.message="baseline sysmon: baselining system monitor services declared on local system. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\logman query counter  >  %fs.CmdbDir%\%baseline.OutputTmpFile%
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline sysmon: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline sysmon: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline sysmon: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline sysmon: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current system monitor services declared on local system:
    @echo.

    %sys.ColorDark%

    %fs.BinDir%\logman query counter

    if %errorlevel% NEQ %EL_LOGMAN_OK% goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline sysmon: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :EXIT
      ...
      set event.message="baseline sysmon: system monitor services declared on local system baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline sysmon] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline sysmon: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline sysmon: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:sysmon [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.sysmon -l:%SET_LOGFILE% -trap:yes
