...
@rem Date:        2009/01/18
@rem Script name: baseline autorun
@rem ------------------------------------------------------------------------------------
...
@rem
@rem Dependencies:
@rem   autorunsc.exe   - www.sysinternals.com (v8.41)
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline autorun <-a:{yes|no}>
@rem                                  <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in autorun.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  set EL_AUTORUNSC_OK=0
  set EL_AUTORUNSC_ERROR=1
  ...
  for %%i in (%fs.LogsDir%\autorun-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=autorun-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\autorun.ini      set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\autorun.ini
    @echo #
  ...
  if not exist %fs.BinDir%\autorunsc.exe                set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\autorun.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.autorun
    ...
    set event.message="baseline autorun: baselining auto-run system items. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\autorunsc -a  >  %fs.CmdbDir%\%baseline.OutputTmpFile%

    if %errorlevel% NEQ %EL_AUTORUNSC_OK% (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline autorun: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline autorun: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline autorun: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline autorun: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current auto-run system items:
    @echo.

    %sys.ColorDark%

    %fs.BinDir%\autorunsc -a

    if %errorlevel% NEQ %EL_AUTORUNSC_OK% goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline autorun: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :HELP
      ...
      @echo.
      @echo   baseline autorun [-a:{yes^|no}]
      @echo                    [-sr:{yes^|no}]
      @echo.
    ...
    :EXIT
      ...
      set event.message="baseline autorun: auto-run system items baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline autorun] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline autorun: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline autorun: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:autorun [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.autorun -l:%SET_LOGFILE% -trap:yes
