...
@rem Date:        2009/01/18
@rem Script name: baseline msbsa
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines the system audit conducted by MSBSA.
@rem
@rem Dependencies:
@rem   mbsacli.exe     - www.microsoft.com (v2.0)
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline msbsa <-a:{yes|no}>
@rem                                <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in msbsa.ini]
@rem   fs.MsBSADir:  [defined in msbsa.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\msbsa-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=msbsa-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\msbsa.ini        set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\msbsa.ini) do (
      if /i %%i EQU fs.ReportDir set fs.ReportDir=%%j
      if /i %%i EQU fs.MsBSADir  set fs.MsBSADir=%%j
    )

    set fs.MsBSADir=%fs.MsBSADir:"=%
    if /i "%fs.MsBSADir:~-1%" EQU "\" set fs.MsBSADir=%fs.MsBSADir:~0,-1%
    if not exist "!fs.MsBSADir!" goto HELP
    ...
    if not exist !fs.MsBSADir!\mbsacli.exe                set input.ValueIsOk=%STATUS_OFF%
    ...
    set baseline.OutputFileRootName=baseline.msbsa
    ...
    set event.message="baseline msbsa: baselining MSBSA output. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    set mbsacli.FailCode=%EL_MBSACLI_OK%
    @!fs.MsBSADir!\mbsacli  >  %fs.CmdbDir%\raw-%baseline.OutputTmpFile%
    if {!errorlevel!} NEQ {%EL_MBSACLI_OK%} set mbsacli.FailCode=%EL_MBSACLI_ERROR%

    type %fs.CmdbDir%\raw-%baseline.OutputTmpFile% | %fs.BinDir%\findstr /v /g:%fs.InstallDir%\msbsa.excluded.ini  >  %fs.CmdbDir%\%baseline.OutputTmpFile%
    del /q /f %fs.CmdbDir%\raw-%baseline.OutputTmpFile%                                                            >  nul

    @echo.
    %sys.ColorNormal%

    if {!mbsacli.FailCode!} NEQ {%EL_MBSACLI_OK%} (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline msbsa: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline msbsa: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline msbsa: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline msbsa: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current MSBSA audit results:
    @echo.
    ...
    if %errorlevel% NEQ %EL_MBSACLI_OK% goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline msbsa: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :EXIT
      ...
      set event.message="baseline msbsa: MSBSA output baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline msbsa] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline msbsa: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline msbsa: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:msbsa [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.msbsa -l:%SET_LOGFILE% -trap:yes
