...
@rem Date:        2009/01/18
@rem Script name: baseline el.triggers
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines el.triggers settings.
@rem
@rem Dependencies:
@rem   eventtriggers.exe - win2k3 reskit
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline el.triggers <-a:{yes|no}>
@rem                                      <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in el.triggers.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
@rem     - <fs.InstallDir> : el.triggers.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\el.triggers-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=el.triggers-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\el.triggers.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\el.triggers.ini
    @echo #
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\el.triggers.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.el.triggers
    ...
    set event.message="baseline el.triggers: baselining event triggers settings. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.BinDir%\eventtriggers /query /v  >  %fs.CmdbDir%\%baseline.OutputTmpFile%
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline el.triggers: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline el.triggers: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline el.triggers: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline el.triggers: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # current event triggers settings:
    @echo.

    %sys.ColorDark%

    %fs.BinDir%\eventtriggers /query /v

    if %errorlevel% NEQ %EL_EVENTTRIGGERS_OK% goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline el.triggers: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :HELP
      ...
      @echo.
      @echo   baseline el.triggers [-a:{yes^|no}]
      @echo                        [-sr:{yes^|no}]
      @echo.
    ...
    :EXIT
      ...
      set event.message="baseline el.triggers: event triggers settings baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline el.triggers] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline el.triggers: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        %sys.ColorRed%
        ...
        set event.message="baseline el.triggers: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:el.triggers [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.el.triggers -l:%SET_LOGFILE% -trap:yes
