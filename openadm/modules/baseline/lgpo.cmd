...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Baselines contents of the Local System Group Policy Object.
@rem
@rem Dependencies:
@rem   secedit.exe       - win2k3 server
@rem   lgpo.inf          - openadm
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline lgpo <-a:{yes|no}>
@rem                               <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in lgpo.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\lgpo-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=lgpo-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\lgpo.ini         set input.ValueIsOk=%STATUS_OFF%
  ...
  if not exist %fs.InstallDir%\lgpo.inf                 set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\lgpo.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.lgpo
    ...
    set event.message="baseline lgpo: baselining system against local GPO. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    set secedit.FailCode=%EL_STATUS_OK%
    %fs.BinDir%\secedit /analyze /db %fs.CmdbDir%\%baseline.OutputFileRootName%.sdb /cfg %fs.InstallDir%\lgpo.inf /overwrite /log %fs.CmdbDir%\%baseline.OutputTmpFile% /quiet
    if {!errorlevel!} GTR {%EL_SECEDIT_OK%} set secedit.FailCode=%EL_STATUS_ERROR%

    @echo.
    %sys.ColorNormal%

    if {!secedit.FailCode!} NEQ {%EL_STATUS_OK%} (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline lgpo: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline lgpo: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline lgpo: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline lgpo: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # ------------------------------------------------------------------------
    %sys.ColorNormal%
    @echo # system audit against local GPO:
    @echo.
    ...
    type %fs.CmdbDir%\%baseline.OutputTmpFile%
    del /q /f %fs.CmdbDir%\%baseline.OutputTmpFile%  >  nul

    %sys.ColorNormal%

    if {!secedit.FailCode!} NEQ {%EL_STATUS_OK%} goto ERROR
    ...
    :ERROR
      ...
      set event.message="baseline lgpo: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :EXIT
      ...
      set event.message="baseline lgpo: system baselined against local GPO. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline lgpo] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline lgpo: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline lgpo: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:lgpo [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.lgpo -l:%SET_LOGFILE% -trap:yes
