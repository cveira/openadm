...
@rem Date:        2009/01/18
@rem Script name: baseline sys.inventory
@rem ------------------------------------------------------------------------------------
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\baseline sys.inventory <-a:{yes|no}>
@rem                                        <-sr:{yes|no}>
@rem
@rem Disk locations:
...
@rem   fs.ReportDir:  [defined in sys.inventory.ini]
@rem ------------------------------------------------------------------------------------
@rem Usage notes:
...
@rem     - <fs.InstallDir> : sys.inventory.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\sys.inventory-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=sys.inventory-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.InstallDir%\sys.inventory.ini set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\sys.inventory.ini
    @echo #
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\sys.inventory.ini) do (
    ...
    set baseline.OutputFileRootName=baseline.sys.inventory
    ...
    set event.message="baseline sys.inventory: baselining system inventory. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    call :BuildInventoryDB
    if /i %report.FormatIsText% NEQ %STATUS_YES% goto EXIT

    set BuildInventory.FailCode=%EL_STATUS_OK%
    call :BuildInventoryReports

    if %BuildInventory.FailCode% EQU %EL_STATUS_OK% copy /v /y %fs.ReportDir%\%report.TxtFile% %fs.CmdbDir%\%baseline.OutputTmpFile%


    if %BuildInventory.FailCode% NEQ %EL_STATUS_OK% (
      goto ERROR
    ) else (
    ...
    if {!errorlevel!} NEQ {%EL_DIFF_IDENTICAL%} (
      ...
      set event.message="baseline sys.inventory: differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline sys.inventory: differences have been notified. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline sys.inventory: unable to send mail notification about the differences found. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul
      )
      ...
      set event.message="baseline sys.inventory: no differences detected. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t information /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if %BuildInventory.FailCode% EQU %EL_STATUS_OK% type %fs.ReportDir%\%report.TxtFile%
    ...
    :ERROR
      ...
      set event.message="baseline sys.inventory: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    :HELP
      ...
      @echo.
      @echo   baseline sys.inventory [-a:{yes^|no}]
      @echo                          [-sr:{yes^|no}]
      @echo.
    ...
    :EXIT
      ...
      set event.message="baseline sys.inventory: system inventory baselined. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul

    copy /v /y %fs.LogsDir%\%SET_LOGFILE% %fs.ReportDir%  >  nul

    if /i %VALUE_SENDREPORTMODE% EQU %STATUS_YES% (
      set mail.subject=[baseline sys.inventory] report
      ...
      if {!errorlevel!} EQU %EL_STATUS_OK% (
        ...
        set event.message="baseline sys.inventory: e-mail report has been sent. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t warning /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul
      ) else (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
        ...
        set event.message="baseline sys.inventory: unable to send e-mail report. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd baseline.sys.inventory -l:%SET_LOGFILE% -trap:yes
...
:BuildInventoryDB
  ...
  setlocal enabledelayedexpansion
    ...
    for /f "eol=# tokens=1,2,* delims=%FIELD_DELIMITER%" %%i in (%fs.InstallDir%\sys.inventory.extensions.ini) do (
      if /i {%%i} EQU {hardware.main} (
        @echo      + catetory: %%j

        @echo ^<category name="%%j"^>                                                >> %fs.ReportDir%\%db.XmlFile%
        wmic /locale:ms_409 %%j get %%k /format:"%parser.xml%"                       >> %fs.ReportDir%\%db.XmlFile%
    ...
    @echo      + catetory: product

    @echo ^<category name="product"^>                                                >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 product list fulll /format:"%parser.xml%"                    >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%
    ...
    @echo      + catetory: dcomapp

    @echo ^<category name="dcomapp"^>                                                >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 dcomapp list brief /format:"%parser.xml%"                    >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%
    ...
    @echo      + catetory: nicconfig

    @echo ^<category name="nicconfig"^>                                              >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 nicconfig list full /format:"%parser.xml%"                   >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%

    @echo      + catetory: printerconfig

    @echo ^<category name="printerconfig"^>                                          >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 printerconfig list full /format:"%parser.xml%"               >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%

    @echo      + catetory: startup

    @echo ^<category name="startup"^>                                                >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 startup list full /format:"%parser.xml%"                     >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%

    @echo      + catetory: recoveros

    @echo ^<category name="recoveros"^>                                              >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 recoveros list full /format:"%parser.xml%"                   >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%

    @echo      + catetory: nteventlog

    @echo ^<category name="nteventlog"^>                                             >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 nteventlog list full /format:"%parser.xml%"                  >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%

    @echo      + catetory: pagefileset

    @echo ^<category name="pagefileset"^>                                            >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 pagefileset list full /format:"%parser.xml%"                 >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%
    ...
    @echo      + catetory: useraccount

    @echo ^<category name="useraccount"^>                                            >> %fs.ReportDir%\%db.XmlFile%
    wmic /locale:ms_409 useraccount list full /format:"%parser.xml%"                 >> %fs.ReportDir%\%db.XmlFile%
    @echo ^</category^>                                                              >> %fs.ReportDir%\%db.XmlFile%
  ...
  setlocal enabledelayedexpansion
    ...
    if /i {%report.FormatIsText%} EQU {%STATUS_YES%} (
      ...
      if {!errorlevel!} EQU {%EL_XML_ERROR%} (
        set event.message="baseline sys.inventory: unable to transform the inventory database into a text file. [%fs.ReportDir%\%db.XmlFile%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul

        %sys.ColorRed%

        @echo.
        @echo # ERROR: unable to transform the inventory database into a text file.
        @echo #
        ...
        set BuildInventory.FailCode=%EL_STATUS_ERROR%
      ) else (
        @echo.
    ...
    if /i {%report.FormatIsHTML%} EQU {%STATUS_YES%} (
      ...
      if {!errorlevel!} EQU {%EL_XML_ERROR%} (
        set event.message="baseline sys.inventory: unable to transform the inventory database into an HTML file. [%fs.ReportDir%\%db.XmlFile%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "baseline:sys.inventory [%SET_SSNSEQ%]" /d !event.message! > nul

        %sys.ColorRed%

        @echo.
        @echo # ERROR: unable to transform the inventory database into an HTML file.
        @echo #
        @echo #   status database:  %db.XmlFile%
