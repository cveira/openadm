...
@rem Date:        2009/01/18
@rem Script name: install folder.tree
@rem ------------------------------------------------------------------------------------
...
@rem   folder.tree.cmd - openadm [folder.tree modules]
...
@rem
@rem Usage:
@rem   <fs.SysOpDir>\install folder.tree <-id:{<group-label>|list|xlist}>
@rem
@rem Disk locations:
...
@rem     - <fs.InstallDir> : folder.tree.<group-label>.ini
@rem     - <%CD%>          : folder.tree.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\folder.tree-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=folder.tree-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if exist %CD%\folder.tree.%VALUE_LABELID%.ini             set input.ValueIsOk=%STATUS_ON%
    if exist %fs.InstallDir%\folder.tree.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   first file location:  "%CD%\folder.tree.%VALUE_LABELID%.ini"
      @echo #   second file location: "%fs.InstallDir%\folder.tree.%VALUE_LABELID%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\folder.tree.%VALUE_LABELID%.ini"

    if exist "%CD%\folder.tree.%VALUE_LABELID%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\folder.tree.%VALUE_LABELID%.ini"
    )
  )
  ...
  :MAIN
    ...
    set event.message="install folder.tree: [%VALUE_LABELID%] starting system folder.tree. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "install:folder.tree [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in ("%CD%\folder.tree.*.ini") do (
        @echo   + %%i
      )
      ...
      for %%i in (%fs.InstallDir%\folder.tree.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\folder.tree.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\folder.tree.*.ini"
    ...
    for /f "eol=# tokens=*" %%i in (!fs.IniDir!\folder.tree.%VALUE_LABELID%.ini) do (
      @echo   + %%i
      md %%i  >  nul
      ...
      if %errorlevel% NEQ %EL_CMD_OK% (
        ...
        set event.message="install folder.tree: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "install:folder.tree [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
      )
    )
    ...
    :HELP
      ...
      @echo.
      @echo   install folder.tree [-id:{[group-label]^|list^|xlist}]
      @echo.
    ...
    :EXIT
      ...
      set event.message="install folder.tree: [%VALUE_LABELID%] system folder.tree finished. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "install:folder.tree [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
