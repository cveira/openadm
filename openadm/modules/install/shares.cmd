...
@rem Date:        2009/01/18
@rem Script name: install shares
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Creates OpenADM System Operations Framework shared resources on the local system.
@rem
@rem Dependencies:
...
@rem   alert.cmd       - openadm [install.shares alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\install shares <-id:{<group-label>|list|xlist}>
@rem                                <-mode:{exec|query}>
@rem                                <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem   install shares -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : shares.<group-label>.ini
@rem     - <%CD%>          : shares.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\shares-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=shares-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if exist %CD%\shares.%VALUE_LABELID%.ini             set input.ValueIsOk=%STATUS_ON%
    if exist %fs.InstallDir%\shares.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   first file location:  "%CD%\shares.%VALUE_LABELID%.ini"
      @echo #   second file location: "%fs.InstallDir%\shares.%VALUE_LABELID%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\shares.%VALUE_LABELID%.ini"

    if exist "%CD%\shares.%VALUE_LABELID%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\shares.%VALUE_LABELID%.ini"
    )
  )
  ...
  :MAIN
    ...
    set event.message="install shares: [%VALUE_LABELID%] installing OpenADM System Operations Framework shared resources. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "install:shares [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\shares.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\shares.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\shares.*.ini"
    ...
    @echo # current state of OpenADM System Operations Framework shared resources environment:
    @echo.
    ...
    if /i %VALUE_MODE% EQU %VALUE_MODE_EXEC% (
      ...
      @echo # ------------------------------------------------------------------------
      %sys.ColorNormal%
      @echo # installing OpenADM System Operations Framework shared resources:
      @echo.

      for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (!fs.IniDir!\shares.%VALUE_LABELID%.ini) do (
        @echo   + shared resource: [%%i] "%%~j"
        @echo.

        %sys.ColorDark%

        net share %%i="%%~j" /unlimited
        ...
        if %errorlevel% NEQ %EL_NET_OK% (
          ...
          @echo #
          @echo #   shared resource name:     %%i
          @echo #   shared resource location: "%%~j"
          @echo.

          %sys.ColorNormal%

          set event.message="install shares: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "install:shares [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!  >  nul
        )
      ...
      @echo # ------------------------------------------------------------------------
      %sys.ColorNormal%
      @echo # final state of OpenADM System Operations Framework shared resources:
      @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   install shares [-id:{[group-label]^|list^|xlist}]
      ...
      @echo.
      @echo     install shares -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="install shares: [%VALUE_LABELID%] OpenADM System Operations Framework shared resources environment installed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "install:shares [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd install.shares -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
