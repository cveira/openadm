...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Creates OpenADM System Operations Framework path environment on the local system.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   list.exe        - win2k3 reskit
@rem   pathman.exe     - win2k3 reskit
...
@rem   alert.cmd       - openadm [install.path alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\install path <-id:{<group-label>|list|xlist}>
@rem                              <-mode:{exec|query}>
@rem                              <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem   install path -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : path.<group-label>.ini
@rem     - <%CD%>          : path.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\path-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=path-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.BinDir%\pathman.exe                  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if exist %CD%\path.%VALUE_LABELID%.ini             set input.ValueIsOk=%STATUS_ON%
    if exist %fs.InstallDir%\path.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   first file location:  "%CD%\path.%VALUE_LABELID%.ini"
      @echo #   second file location: "%fs.InstallDir%\path.%VALUE_LABELID%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\path.%VALUE_LABELID%.ini"

    if exist "%CD%\path.%VALUE_LABELID%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\path.%VALUE_LABELID%.ini"
    )
  )
  ...
  :MAIN
    ...
    set event.message="install path: [%VALUE_LABELID%] installing OpenADM System Operations Framework path environment. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "install:path [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\path.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\path.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\path.*.ini"
    ...
    @echo # current state of OpenADM System Operations Framework path environment:
    @echo.
    ...
    if /i %VALUE_MODE% EQU %VALUE_MODE_EXEC% (
      ...
      @echo # ------------------------------------------------------------------------
      %sys.ColorNormal%
      @echo # installing OpenADM System Operations Framework path environment:
      @echo.

      for /f "eol=# tokens=* delims=%FIELD_DELIMITER%" %%i in (!fs.IniDir!\path.%VALUE_LABELID%.ini) do (
        @echo   + path: "%%~i"
        @echo.

        %sys.ColorDark%

        %fs.BinDir%\pathman /as "%%~i"

        if %errorlevel% NEQ %EL_PATHMAN_OK% (
          @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
          ...
          set event.message="install path: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "install:path [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!  >  nul
        )
      ...
      @echo # ------------------------------------------------------------------------
      %sys.ColorNormal%
      @echo # final state of OpenADM System Operations Framework path environment:
      @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   install path [-id:{[group-label]^|list^|xlist}]
      ...
      @echo.
      @echo     install path -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="install path: [%VALUE_LABELID%] OpenADM System Operations Framework path environment installed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "install:path [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd install.path -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
