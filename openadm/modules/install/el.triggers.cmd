...
@rem Date:        2009/01/18
@rem Script name: install el.triggers
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Creates OpenADM System Operations Framework event log triggers on the local system.
@rem
@rem Dependencies:
@rem   eventcreate.exe   - win2k3 server
@rem   list.exe          - win2k3 reskit
@rem   eventtriggers.exe - win2k3 server
...
@rem   alert.cmd         - openadm [install.el.triggers alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\install el.triggers <-id:{<group-label>|list|xlist}>
@rem                                     <-mode:{exec|query}>
@rem                                     <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem   install el.triggers -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.ConfDir>    : el.iddb.ini
@rem     - <fs.InstallDir> : el.triggers.<group-label>.ini
@rem     - <%CD%>          : el.triggers.<group-label>.ini
@rem
...
setlocal enabledelayedexpansion
  ...
  set EL_EVENTTRIGGERS_OK=0
  set EL_EVENTTRIGGERS_ERROR=1
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if /i "%VALUE_ALERTMODE%" EQU 2%STATUS_YES%"    set input.ValueIsOk=%STATUS_ON%
    if /i "%VALUE_ALERTMODE%" EQU 2%STATUS_NO%"     set input.ValueIsOk=%STATUS_ON%
  ...
  for %%i in (%fs.LogsDir%\el.triggers-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=el.triggers-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.BinDir%\eventtriggers.exe            set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.BinDir%\eventtriggers.exe
    @echo #
    @echo # aborting program.
    @echo.
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if exist %CD%\el.triggers.%VALUE_LABELID%.ini             set input.ValueIsOk=%STATUS_ON%
    if exist %fs.InstallDir%\el.triggers.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   first file location:  "%CD%\el.triggers.%VALUE_LABELID%.ini"
      @echo #   second file location: "%fs.InstallDir%\el.triggers.%VALUE_LABELID%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\el.triggers.%VALUE_LABELID%.ini"

    if exist "%CD%\el.triggers.%VALUE_LABELID%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\el.triggers.%VALUE_LABELID%.ini"
    )
  )
  ...
  :MAIN
    ...
    set event.message="install el.triggers: [%VALUE_LABELID%] installing OpenADM System Operations Framework event log triggers. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "install:el.triggers [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in ("%CD%\el.triggers.*.ini") do (
        @echo   + %%i
      )
      ...
      for %%i in (%fs.InstallDir%\el.triggers.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\el.triggers.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\el.triggers.*.ini"
    ...
    @echo # current state of OpenADM System Operations Framework event log triggers:
    @echo.

    %fs.BinDir%\eventtriggers /query
    ...
    if /i %VALUE_MODE% EQU %VALUE_MODE_EXEC% (
      ...
      @echo # ------------------------------------------------------------------------
      %sys.ColorNormal%
      @echo # installing OpenADM System Operations Framework event log triggers:
      @echo.

      %sys.ColorDark%

      for /f "eol=# tokens=1,2,3,* delims=%FIELD_DELIMITER%" %%a in (!fs.IniDir!\el.triggers.%VALUE_LABELID%.ini) do (
        @echo   + trigger: %%a [%%b]
        @echo.

        %fs.BinDir%\eventtriggers /create /tr %%a /ru %%b /rp %%c %%d

        if %errorlevel% NEQ %EL_EVENTTRIGGERS_OK% (
          @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
          ...
          @echo #   job name:       %%a
          @echo #   job user:       %%b
          @echo #   job parameters: %%d
          @echo.

          %sys.ColorNormal%

          set event.message="install el.triggers: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "install:el.triggers [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!  >  nul
        )
      ...
      @echo # ------------------------------------------------------------------------
      %sys.ColorNormal%
      @echo # final state of OpenADM System Operations Framework event log triggers:
      @echo.
    ...
    :HELP
      ...
      @echo.
      @echo   install el.triggers [-id:{[group-label]^|list^|xlist}]
      ...
      @echo.
      @echo     install el.triggers -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="install el.triggers: [%VALUE_LABELID%] OpenADM System Operations Framework event log installed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "install:el.triggers [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd install.el.triggers -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
