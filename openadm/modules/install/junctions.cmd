...
@rem Date:        2009/01/18
@rem Script name: install junctions
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Creates OpenADM System Operations Framework FS junction points on the local system.
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   list.exe        - win2k3 reskit
@rem   linkd.exe       - win2k3 reskit
@rem   hlscan.exe      - win2k3 reskit
...
@rem   alert.cmd       - openadm [install.junctions alert module]
@rem   sctl.cmd        - openadm [scm sctl module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\install junctions <-id:{<group-label>|list|xlist}>
@rem                                   <-mode:{exec|query}>
@rem                                   <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem   install junctions -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : junctions.<group-label>.ini
@rem     - <%CD%>          : junctions.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\junctions-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=junctions-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.BinDir%\hlscan.exe                   set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if exist %CD%\junctions.%VALUE_LABELID%.ini             set input.ValueIsOk=%STATUS_ON%
    if exist %fs.InstallDir%\junctions.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   first file location:  "%CD%\junctions.%VALUE_LABELID%.ini"
      @echo #   second file location: "%fs.InstallDir%\junctions.%VALUE_LABELID%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\junctions.%VALUE_LABELID%.ini"

    if exist "%CD%\junctions.%VALUE_LABELID%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\junctions.%VALUE_LABELID%.ini"
    )
  )
  ...
  :MAIN
    ...
    set event.message="install junctions: [%VALUE_LABELID%] installing OpenADM System Operations Framework FS junctions. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "install:junctions [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\junctions.*.ini) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\junctions.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\junctions.*.ini"
    ...
    if /i %VALUE_MODE% EQU %VALUE_MODE_QUERY% (
      @echo # current state of OpenADM System Operations Framework FS junctions:
      @echo.

      %sys.ColorDark%

      %fs.BinDir%\hlscan /all
    ...
    if /i %VALUE_MODE% EQU %VALUE_MODE_EXEC% (
      ...
      call %fs.SysOpDir%\sctl scm -id:install.junctions -cmd:stop -a:no -w:15

      @echo.
      %sys.ColorBright%
      @echo # ------------------------------------------------------------------------
      %sys.ColorNormal%
      @echo # installing OpenADM System Operations Framework FS junctions:
      @echo.

      for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (!fs.IniDir!\junctions.%VALUE_LABELID%.ini) do (
        @echo   + junction definition: [%%i] "%%~j"
        @echo.
        ...
        if %errorlevel% NEQ %EL_LINKD_OK% (
          ...
          @echo #   target folder: "%%~j"
          @echo.

          %sys.ColorNormal%

          set event.message="install junctions: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
          %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "install:junctions [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!  >  nul
        )
      ...
      call %fs.SysOpDir%\sctl scm -id:install.junctions -cmd:start -a:no -w:15
    )
    ...
    :HELP
      ...
      @echo.
      @echo   install junctions [-id:{[group-label]^|list^|xlist}]
      ...
      @echo.
      @echo     install junctions -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="install junctions: [%VALUE_LABELID%] OpenADM System Operations Framework FS junctions installed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "install:junctions [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul

    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd install.junctions -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
