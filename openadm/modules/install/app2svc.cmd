...
@rem Date:        2009/01/18
@rem Script name: install app2svc
@rem ------------------------------------------------------------------------------------
...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Installs an stand alone application as a service on the local system.
@rem
@rem Dependencies:
@rem   eventcreate.exe       - win2k3 server
@rem   list.exe              - win2k3 reskit
@rem   reg.exe               - win2k3 server
...
@rem   install.cmd           - openadm [services module]
@rem   alert.cmd             - openadm [install.app2svc alert module]
@rem
@rem Usage:
@rem   <fs.SysOpDir>\install app2svc <-id:{<group-label>|list|xlist}>
@rem                                 <-mode:{exec|query}>
@rem                                 <-a:{yes|no}>
@rem
@rem Disk locations:
...
@rem   install app2svc -id:{list|xlist}
@rem
@rem Important notes:
@rem   It uses several text files located on:
@rem     - <fs.InstallDir> : app2svc.<group-label>.ini
@rem     - <fs.InstallDir> : app2svc.<group-label>.reg
@rem     - <%CD%>          : app2svc.<group-label>.ini
@rem     - <%CD%>          : app2svc.<group-label>.ini
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\app2svc-%VALUE_LABELID%-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=app2svc-%VALUE_LABELID%-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.BinDir%\reg.exe                      set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i %context.SkipExec% EQU %STATUS_OFF% (
    ...
    if exist %CD%\app2svc.%VALUE_LABELID%.ini             set input.ValueIsOk=%STATUS_ON%
    if exist %fs.InstallDir%\app2svc.%VALUE_LABELID%.ini  set input.ValueIsOk=%STATUS_ON%
    ...
    if /i !input.ValueIsOk! EQU %STATUS_OFF% (
      ...
      @echo # ERROR: input INI file does not exist.
      @echo #
      @echo #   first file location:  "%CD%\app2svc.%VALUE_LABELID%.ini"
      @echo #   second file location: "%fs.InstallDir%\app2svc.%VALUE_LABELID%.ini"
      @echo #
      @echo # aborting program.
      @echo.
    ...
    set fs.IniFile="%fs.InstallDir%\app2svc.%VALUE_LABELID%.ini"

    if exist "%CD%\app2svc.%VALUE_LABELID%.ini" (
      set fs.IniDir=%CD%
      set fs.IniFile="%CD%\app2svc.%VALUE_LABELID%.ini"
    )


    if not exist "!fs.IniDir!\app2svc.%VALUE_LABELID%.reg" (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%fs.LogFile:.log=.exit%
      ...
      @echo #
      @echo #   file location:  "!fs.IniDir!\app2svc.%VALUE_LABELID%.reg"
      @echo #
      @echo # aborting program.
      @echo.
      ...
      set fs.RegFile="!fs.IniDir!\app2svc.%VALUE_LABELID%.reg"
    )
  )
  ...
  :MAIN
    ...
    for /f "eol=# tokens=1,* delims=%FIELD_DELIMITER%" %%i in (!fs.IniDir!\app2svc.%VALUE_LABELID%.ini) do (
      if /i %%i EQU SourceFolder set service.SourceFolder=%%j
      if /i %%i EQU TargetFolder set service.TargetFolder=%%j
    )

    set event.message="install app2svc: [%VALUE_LABELID%] installing target application as a local service. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "install:app2svc [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_LIST% (
      ...
      for %%i in (%fs.InstallDir%\app2svc.*) do (
        @echo   + %%i
      )
    ...
    if /i %VALUE_LABELID% EQU %VALUE_LABELID_XLIST% (
      ...
      %fs.BinDir%\list "%CD%\app2svc.*.ini"
      %fs.BinDir%\list "%fs.InstallDir%\app2svc.*.ini"
    ...
    if /i {%VALUE_MODE%} EQU {%VALUE_MODE_EXEC%} (
      @echo # installing target application as a local service:
      @echo.
      ...
      copy /v /y %service.SourceFolder%\*.* %service.TargetFolder% 2>&1 > nul
      dir /b %service.TargetFolder%\*.*
      ...
      call %fs.SysOpDir%\install.cmd services -id:%VALUE_LABELID% -mode:exec -a:no
      ...
      if %errorlevel% NEQ %EL_STATUS_OK% (
        ...
        set event.message="install app2svc: [%VALUE_LABELID%] unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
        %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "install:app2svc [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message!  >  nul
      )
      ...
      %fs.BinDir%\reg import %fs.InstallDir%\app2svc.%VALUE_LABELID%.reg

      if {%errorlevel%} NEQ {%EL_REG_OK%} (
        @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
    ...
    :HELP
      ...
      @echo.
      @echo   install app2svc [-id:{[group-label]^|list^|xlist}]
      ...
      @echo.
      @echo     install app2svc -id:{list^|xlist}
      @echo.
    ...
    :EXIT
      ...
      set event.message="install app2svc: [%VALUE_LABELID%] target application as a local service installed. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "install:app2svc [%VALUE_LABELID%] [%SET_SSNSEQ%]" /d !event.message! > nul


    if /i %VALUE_ALERTMODE% EQU %STATUS_YES% call %fs.SysOpDir%\alert.cmd install.app2svc -l:%SET_LOGFILE% -id:%VALUE_LABELID% -trap:no
