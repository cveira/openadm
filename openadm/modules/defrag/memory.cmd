...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Forces the system to page memory to disk in order to free main system memory
@rem
@rem Dependencies:
@rem   eventcreate.exe - win2k3 server
@rem   clearmem.exe    - win2k3 reskit
...
@rem
@rem Usage:
@rem   <fs.InstallDir>\defrag memory
@rem
@rem Disk locations:
...
setlocal enabledelayedexpansion
  ...
  set EL_CLEARMEM_OK=0
  set EL_CLEARMEM_ERROR=1
  ...
  for %%i in (%fs.LogsDir%\memory-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=memory-%yy%%mm%%dd%-%fs.LogFileCount%.log
  ...
  if not exist %fs.BinDir%\clearmem.exe                 set input.ValueIsOk=%STATUS_OFF%
  ...
  :MAIN
    ...
    set event.message="defrag memory: defragmenting system memory. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "defrag:memory [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    @echo # defragmenting system memory
    @echo.
    ...
    if %errorlevel% NEQ %EL_CLEARMEM_OK% (
      @echo %EL_STATUS_ERROR% >> %fs.TmpDir%\%SET_LOGFILE:.log=.exit%
      ...
      set event.message="defrag memory: unexpected operation error. [%fs.LogsDir%\%SET_LOGFILE%]"
      %fs.BinDir%\eventcreate /id %event.id% /l application /t error /so "defrag:memory [%SET_SSNSEQ%]" /d !event.message! > nul
    )
    ...
    set event.message="defrag memory: system memory defragmented. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "defrag:memory [%SET_SSNSEQ%]" /d !event.message! > nul
