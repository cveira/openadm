...
@rem ------------------------------------------------------------------------------------
@rem Description:
@rem   Catches every event log delete operation at generation time and copies
@rem   affected events into another text file in order to increase system security.
@rem
@rem Dependencies:
@rem   rtelprotect.vbs - dotpi.com
...
@rem     - <fs.InstallDir> : rtelprotect.ini
@rem
...
setlocal enabledelayedexpansion
  ...
  for %%i in (%fs.LogsDir%\rtelprotect-%yy%%mm%%dd%*.log) do (
    set /a fs.LogFileCount+=1
  )

  set fs.LogFile=rtelprotect-%yy%%mm%%dd%-%fs.LogFileCount%.log


  set input.ValueIsOk=%STATUS_ON%

  if not exist %fs.ConfDir%\el.iddb.ini         set input.ValueIsOk=%STATUS_OFF%
  if not exist %fs.InstallDir%\rtelprotect.ini  set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\rtelprotect.ini
    @echo #
  ...
  if not exist %fs.InstallDir%\rtelprotect.vbs          set input.ValueIsOk=%STATUS_OFF%
  ...
  if /i !input.ValueIsOk! EQU %STATUS_OFF% (
    ...
    @echo #   %fs.InstallDir%\rtelprotect.vbs
    @echo #
    @echo # aborting program.
    @echo.
  ...
  :MAIN
    ...
    for /f "eol=#" %%i in (%fs.InstallDir%\rtelprotect.ini) do set openadm.CollectOptions=%%i

    set event.message="rtelprotect: starting real-time monitoring. [audit profile is: %CollectOptions%]. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.BeginId% /l application /t information /so "service:rtelprotect [%SET_SSNSEQ%]" /d !event.message! > nul
    ...
    %fs.InstallDir%\rtelprotect %openadm.CollectOptions%
    ...
    set event.message="rtelprotect: real-time monitoring stopped. [audit profile was: %CollectOptions%]. [%fs.LogsDir%\%SET_LOGFILE%]"
    %fs.BinDir%\eventcreate /id %event.EndId% /l application /t information /so "service:rtelprotect [%SET_SSNSEQ%]" /d !event.message! > nul
